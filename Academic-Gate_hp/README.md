...
| `html/` | 12 pages. Every `<body>` carries `data-page` (drives nav active state); a reading page adds `class="is-article"` (§35). **勉強の軌跡 is four levels on every branch** — `study.html` (分野一覧) → `study_<field>.html` (分野) → `<subject>.html` (科目) → `<subject>_<chapter>-<section>.html` (記事): `study_physics.html` → `phys-math.html` → `phys-math_1-1.html`, and `study_skills.html` → `memoryverse.html` → `memoryverse_1-1.html` (§37.2, §39.1, §48). **A group and a chapter are `h2` headings, never pages** — get that wrong and you grow a fifth level (§48.1). The 科目 slug is frozen as the URL; display names change without moving files (§48.3). Every level after the first is a **card grid** (§38). **Flat only**: a subdirectory breaks every relative nav link — the migration trigger and recipe are in §37.2. |
