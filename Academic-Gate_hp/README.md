...
```sh
sh Academic-Gate_hp/tools/bump.sh          # r55 → r56 across every page + README
...
- **Assets are cache-busted** with `?v=rNN` query strings in the HTML. Bump the number when
  editing `css`/`js` so browsers refetch. (Current: **`r57`**.) **Do not do this by hand:**
