...
- ★ **未解決の食い違い（§99 で発生）**：第5回の付録A（地球の回の構成案）と付録C（次回の予告）は、
  地球の最初の回を「大陸と海洋、大きな山脈と川」、その後を「気候 → 国と地名 → 産物 → 人びと → 企業と観光地」と
  予告しているが、2-1 で構成を変えた（基準線と錨の11か国を入れ、山脈と川は後へ、2-2 以降は先に地域の回）。
  付録Cの「分かっていること・推計・時点で変わるものを分ける」も §98.1 で採らないと決めた案のまま。
  あわせて、1章の本文は地球の回を「第6回（以降）」と呼んでいる（出現数で 1-1 に3、1-2 に9、1-4 に1、1-5 に9。
  `grep -rn "第6回" html/`）。2章からは章ごとの番号（2-1〜）になったので、「2章（地球の回）」「2-1」に読み替える必要がある。
  **2-1 のコミットでは1章を直していない**。直すときは付録A・Cと呼び名を一緒に、別のコミットで。
- ★ **3章に入る前に決める**：3章を何回に分けるか（M49 の23地域をそのまま使うか、小さい地域をまとめるか）。
  **統合の回の形も一緒に決める**——回の分け方で巡回路の長さが決まるため。2-1 の付録Aには「まだ決めていない」と書いてある
- ★ **第5回の残骸（別のコミットで直す）**：`html/memoryverse_1-5.html` の `<style>` に、メディアクエリの開き行が
  失われた1行 `  .lesson-nav .next,.lesson-nav .mid{text-align:left}}` がある。**すべての幅で無条件に効いている**ので、
  第5回だけ前後リンクの `.next` が右揃えにならない。2-1 には写していない（§99.4）
