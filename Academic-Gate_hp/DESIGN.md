...
「数式が狭い画面でどう振る舞うべきか」という一つの主題なのでまとめて扱う。
段階を分けて進める：**46.1 案A（実装済み）→ 46.2 案B-2（未着手）→ 46.3 C-2（未着手）**。
...
**全量（合計そのまま）だと数式が枠線に重なる。** `.sol` の左端には
`border-left:2px solid var(--accent)` の青いアクセントバーがあり、その上に分数が
乗って明確な悪化になる。8px 残すと枠線に触れない。

| | 1024px のはみ出し | 枠線 |
|---|---|---|
| 現状 | 6 本 / 最大 +127px | — |
| 全量を突き抜け | 3 本 / 最大 +41px | ★ 重なる（不採用） |
| **−8px（採用）** | **4 本 / 最大 +41px** | 触れない |

1本ぶんの差で枠線の破綻を避ける取引になっている。
...
## 46.2 案B-2 — はみ出している数式の縁をフェード（未着手）
...
1・2 を入れたあと 390px の実物を見てから判断する。

## 46.4 State（46.1 のみ working tree — NOT committed/pushed）
更新 `html/phys-math_1-1.html`（`<style>` に CSS 6行＋注記）、`DESIGN.md`。
