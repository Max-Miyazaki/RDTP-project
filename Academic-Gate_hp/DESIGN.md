...
# 81. 巨大惑星は断面で覚える（第3回の巨大惑星4枚）

第2回は「地面のある天体を地図で覚える」回だった。巨大惑星には固体の表面が無く、
**貼れる地図が存在しない**。カードの2層を **［断面］と［球］** に変え、覚える対象を
断面のほうへ置く。

次に足す主要衛星8枚には地面があるので、第2回と同じ **［地図］と［球］** に戻る。
そのため map-view 用の CSS と JS（`mvGrid` / `mvMap` / `mvLinks`）は、この回で
1つも使っていないが**そのまま残してある**。消すと衛星の段階で作り直しになる。

## 81.1 断面の描き方
- 円の4分の1。中心は左下、右上へ開く
- ★ **4枚とも中心・半径・向き・色をそろえる**。カードごとに縮尺を変えると、
  §81.6 の「4つ並べた比較図」も、木星と土星を見くらべる操作も意味を失う
- 開閉の規則は第2回の地図のピンと同じ（§56.1）。閉じているときは層の輪郭と色だけ、
  開くと層の名前（`.lay-lab`）が出る
- ★ `opacity:0` だけでは読み上げとページ内検索に残り、**閉じているのに答えが読める**。
  `visibility:hidden` も落とすこと
- 金属水素の層だけ紫（`rgba(123,77,255,.26)`）にして、有無が一目で分かるようにした。
  トークンの `--teach-tint`（.11）では薄すぎて、比較図で差が読めなかった（実測で .26 に）

## 81.2 数値をどこまで入れたか
層の境界の深さは資料により幅がある。**確かな部分だけを書き、範囲には幅を明記**した。

| 入れた | 入れなかった |
|---|---|
| 木星：雲の層は半径の1%にも満たない | 大気と分子水素の境目の深さ |
| 木星：金属水素の始まりは半径の約0.7〜0.8（資料により幅がある、と明記） | 断定的な単一の値 |
| 土星：金属水素の始まりは半径の約0.5あたり（同上） | 同上 |
| 核：「大きさは未確定」（木星はジュノーの希薄な核） | 核半径の数値 |
| 天王星・海王星：氷のマントルは体積の大半 | 層の境界の深さ |

図の中に「**層の厚みは模式。覚えるのは順番**」を常時出す。これは答えではないので、
閉じていても出したままにする。

## 81.3 ★ 狭い画面では図の中の文字が 5px まで縮む
560px 以下で図中のラベルは **実測 5.1px**。読めない。第2回で地図のラベルを
番号つきの一覧に落としたのと同じ扱いにした（§54.5）。

- ラベル（`.lay-lab`）を畳み、色つきの一覧（`.cut-list`）を図の下に出す
- 図はラベル列（viewBox の x=398 から右）を切り落とし、扇形だけを幅いっぱいに使う。
  `width:170%` ＋ `overflow:hidden`。**170% は 640÷375**（375 は扇形と
  「表面＝1気圧の高さ」までの幅）
- 結果、文字は 11.8px 相当まで回復。390 / 768 / 1024px で横スクロール0を実測

## 81.4 球にピンを置かない
- 木星の大赤斑は**経度が動く**。緯度（南緯約22度）は決まるが経度が定まらない
- 天王星・海王星は可視光でほとんど模様が見えない。**指させる場所が無い**

`.stat.geo[data-lat]` が0件なので、§78 の球のコードが ［地形を開く］ ボタンを
自動で隠す（地球と同じ経路）。**球のコードは1行も変えていない。**

## 81.5 衛星の数の典拠を IAU 小惑星センターに統一
第1回は NASA Space Place を典拠に木星115・土星293と書いていた。第3回では
IAU 小惑星センター（MPC）の確認総数に統一し、**第1回の数値も同じ典拠に揃えた**。

| 天体 | 値 | 典拠 |
|---|---|---|
| 木星 | 101 | 2026年3月16日の MPEC（IAU 告知は同年3月26日） |
| 土星 | 285 | 同上 |
| 天王星 | 29 | 2025年8月に S/2025 U1（JWST）が加わった値 |
| 海王星 | 16 | NASA Science / JPL SSD（2026年9月時点） |

★ 指示は海王星を 16 → 18 だったが、**18 を裏づける資料が見つからなかった**。
NASA Science の海王星の衛星ページ（2026年9月21日更新）と JPL SSD の一覧は
どちらも 16。天王星・海王星には MPC による「確認総数」の発表が見当たらない。
確認できない数を IAU に帰属させるわけにいかないので 16 のままとし、その旨を
本文の注記に書いた。両方の回に「**数は数え方と確認の時点で変わる。覚えるのは
数ではなく、名前のついた主要な衛星のほう**」を入れてある。

## 81.6 枠だけにした節
到達目標・探査の歴史・訓練メニュー・演習は「準備中」の箱だけ。衛星8枚の段階で埋める。
付録Aは改訂表を引き継ぎ、付録Bは第2回の表に「**断面を重ねる**」を**弱**（検証した
研究は確認できていない、設計判断である旨つき）で1行足した。

§02「この回の見方」だけは枠にせず、カードが成立するのに必要な3つの箱
（断面の開閉の規則／球にピンを置かない理由／衛星の数の扱い）を書いた。

## 81.7 State
`d552be6`（r62）。`html/memoryverse_1-3.html` を新規追加、`html/phys-math.html`
以外の html と `README.md` はキャッシュバスターのみ、`js/layout.js` の CRUMBS、
`html/memoryverse.html` のカード、`html/memoryverse_1-1.html` の衛星数。

---

# 82. 土星の球を単一時期のマップに差し替える／天王星に近赤外を1枚

## 82.1 ★ 合成テクスチャは「継ぎはぎ」が見える
最初に採用した土星のテクスチャ（Askaniy / Björn Jónsson, CC BY-SA 3.0）は
**北半球がボイジャー2号（1981年）、南半球がカッシーニ（2004年）**の合成だった。
20年以上離れた画像なので帯の位置も色もそろわず、実物らしく見えない。

差し替え先は **NASA Planetary Data System** の Cassini ISS 全球カラーマップ
（Li, West, Jiang, Knowles 2023）。**2011年8月11日 03:05〜11:40 UTC の1日分**、
カッシーニ5回転ぶんの広角画像から作られている。正距円筒 3601×1801（0.1度／画素）、
パブリックドメイン。Photojournal ではなく PDS 本体にある。

## 82.2 ★ FITS のヘッダと実データで行の向きが逆だった
`LAT_C = '-90:0.1:90'`、`display_settings` は `Line: Bottom to Top`。素直に読むと
**行0が南**になるが、**実データは行0が北**だった。突き合わせた2点：

- 2010〜2011年の大白斑の乱れた帯が**北緯25〜48度**に来ること
- 環に隠れて撮れていない帯が**南緯側**に来ること

どちらも PDS のブラウズ画像と一致する向きを採った。素直に反転すると南北が逆になる。

もう1つ。**列0はモザイクの継ぎ目で暗い**（緑の列平均 176 に対し、ほかは210前後）。
落として 3600 列にしてから、左端が東経180度になるよう半周ぶん回す。
値は**すでに 0〜255 の表示用 RGB**なので、ガンマも正規化もかけない（かけると白ける）。

## 82.3 ★ データの無い帯を埋めたことを明記する
カッシーニから見て環に隠れていた **南緯5〜17度**、および **北緯79度より北・
南緯82度より南**は撮れていない（1801行中318行）。そのままだと球に黒い縞が走る。

上下の色で線形につないで埋めたうえで、**埋めたこと・どこを埋めたかを figcaption と
末尾のクレジットに書いた**。模様は入れていない。加工を黙って行わないこと。

## 82.4 天王星の近赤外は「球にしない1枚」
天王星は**可視光でほとんど模様が見えないのが正しい姿**で、それ自体が見分けどころ。
球は差し替えない。代わりに、可視光で見えないものを補う証拠として、近赤外の静止画を
**1枚だけ**「見え方」の行の直下に置く（`.fig.ir`）。回せる球にはしない。

ジェイムズ・ウェッブ NIRCam、2023年2月6日（ESA/Webb `weic2310a`）。
クレジット `NASA, ESA, CSA, STScI, J. DePasquale (STScI)` ／ CC BY 4.0。
ESA/Webb は**クレジットを省略せず見える形で出すこと**が条件なので、figcaption と
末尾の両方に全文を入れてある。

海王星には足していない。可視光でも白い雲が見えるので、天王星と同じ理由が成り立たない。
足すとしたら理由が変わる（環がどこにも写っていない）ので、そのときに判断する。

## 82.5 球のテクスチャ一覧（この回の4枚）
いずれも正距円筒 2:1、［球］を押したときだけ読み込む。

| 天体 | 出どころ | ライセンス | 加工 |
|---|---|---|---|
| 木星 | NASA/JPL/SSI `PIA07782`（カッシーニ 2000年12月） | パブリックドメイン | 縮小のみ |
| 土星 | NASA PDS / Li ほか 2023（カッシーニ 2011年8月11日） | パブリックドメイン | 縮小＋欠損帯の補間（明記） |
| 天王星 | Solar System Scope | CC BY 4.0 | 縮小のみ |
| 海王星 | Solar System Scope | CC BY 4.0 | 縮小のみ |

海王星は**色が強調されている**（ボイジャー2号の濃い青は画像処理で強められたもので、
補正すると天王星に近い青緑 — Irwin et al. 2024）。暗斑は1989年に見えていたもので、
その後消えた。どちらも figcaption に書いてある。

## 82.6 State
`d2ce1ea`（r63）。`image/memoryverse/saturn-map.jpg` を差し替え、
`image/memoryverse/uranus-ir.jpg` を追加、`html/memoryverse_1-3.html`。
土星のテクスチャは**ファイル名が同じまま中身が変わる**ので、キャッシュバスターの
バンプが必須（r62 → r63）。

---

# 83. 環の並びの図（各カードに1枚＋横断の比較図）

## 83.1 ★ 指示の読み違いで3枚を取りこぼした
「**同じ図を4枚並べて比べられるようにするか、土星だけにするか**」という指示を、
「横断の節に置く4行の比較図」のことだと読み、土星のカードの図＋§08 の比較図で
「両方やる」と報告した。実際は「**各カードに同じ図を1枚ずつ、計4枚**」で、
横断の比較図は追加の選択肢（「〜手もあります」）だった。木星・天王星・海王星の
3枚が抜けたまま合意された形になり、あとから作り直している。

★ **選択肢を示されたときは、選ぶだけでなく「その結果どのファイルに何が増えるか」を
具体的に書いてから着手する。**「土星カードに1枚＋§08に1枚、ほかのカードには置かない」
と書いていれば、その場で食い違いが見つかっていた。

## 83.2 描き方（4枚とも共通）
- 真横から見た並び。左に本体（半円。縦も横も同じ縮尺）、右へ環を順に
- 横は**その惑星の半径を1**とした距離。viewBox 640×396、`X0=36`、`RMAX=4.40`、
  `S=(640-36-34)/4.40`。**4枚とも同じ寸法・同じ目盛**
- 距離は **NSSDC の各 Rings Fact Sheet の `Radius/Eq. radius` 列**をそのまま使う
- 濃淡は**光学的厚みとアルベドの順に合わせた模式**。正確な明るさの比ではない旨を明記。
  D環・G環・E環がほとんど見えないのは、実際に非常に薄いから（D環の光学的厚みは 10⁻⁵）
- 開閉は断面と同じ `.mod.open` を共有。閉じているときは帯の濃淡と距離だけ、
  開くと環の名前が出る
- 全4枚に2つの注記：「**環の厚さはこの図では表せない**」と「**惑星ごとに半径が違うので、
  この図どうしで実距離は比べられない。4つを比べるときは横断の節の比較図へ**」

## 83.3 ★ 図に押す入口が無いと、名前は出ないのと同じ
最初、環の図は `.mod-b` の中のただの `<figure>` だった。開閉できるのは本文の
**ずっと上にある断面の図だけ**で、環の図を見ている人がそこまで戻らないと名前が出ない。
実装はされているのに「名前が出ない」という報告になった。

`<button class="ring-btn">` で包み、「クリックで環の名前を開く／閉じる」の一行を添えた。
★ 断面の `.img-btn` が使っている `mvKeepTop`（カードの上端まで画面を引き戻す）は
**呼ばない**。呼ぶと、いま見ている環の図が画面の外へ飛ぶ（実測：押しても移動量 0px）。
`aria-expanded` は両方のボタンで揃える。

## 83.4 名前の置き方
環が密集すると、ラベルも引き出し線も互いを貫く。当たり判定（線分を1.5px刻みで
文字の矩形に当てる）で確認しながら決めた。

| 天体 | 置き方 |
|---|---|
| 木星 | ハロー環・ゴサマー環は括弧で範囲を示す。★ 括弧の縦線は、ラベルが0段目のときは引かない（文字を貫く） |
| 土星 | 帯の真上に D・C・B・A・F・G・E。カッシーニの間隙は帯の下に琥珀色で |
| 天王星 | ε・ν・μ を個別に。**内側の9本は 1.637〜1.957 に密集して個別に置けない**ので、括弧でまとめて1つのラベルに |
| 海王星 | 環が 2.1〜2.5倍に密集。**L字の引き出し線で1段に並べる**（帯から真上に伸ばし、ラベルの下で横へ折る）。この方式なら縦線がラベルを貫かない |

## 83.5 省いたものは図の下に書く
- 天王星：NSSDC の表にある**12本**を描いている。いちばん内側のζ環は表に無いので省いた
  （カードの本文は13本）
- 海王星：半径の2.50倍にある**名前のない淡い環**も NSSDC にあり、名前なしで描いている

## 83.6 図に名前があるなら、本文で一度は説明する
比較図に「ε環」「主環」の名前が出ているのに、本文に説明が無かった。点検して埋めた。

- 天王星に「**ε環**」の行を新設：細い環のなかで最も明るく最も太い（幅 20〜96km、
  ほかの細い環は1〜11km）／1977年の掩蔽で見つかった5本、地上観測を合わせた9本の
  いずれでもいちばん外側／ν環・μ環はさらに外側で、幅は広いが極めて淡い／
  コーデリアとオフィーリアが羊飼い衛星として粒をつなぎとめている
- 木星の「環」の行に「**主環**」の説明（3つのうち最も濃い／幅 約6,000km）
- ★ 指示にあった「13本のうちいちばん太い」は書けない。**ν環（約3,800km）と
  μ環（約17,000km）のほうが桁違いに広い**（ただし光学的厚みは `<<0.01`）。
  「細い環のなかでいちばん太い」に直した

## 83.7 State
`eb72c3d`（土星の図と §08 の比較図）、`c772ee3`（押す入口、ε環と主環の説明）、
`86bc2d4`（木星・天王星・海王星の3枚、土星の図に注記2行を足して高さを 378→396 に統一）。
いずれも `html/memoryverse_1-3.html` のみ。css / js / 画像に変更が無いので
キャッシュバスターは r64 のまま。

---

# 84 モバイルで数式が溢れる件（物理数学I・共通CSS）

390px / 360px で、物理数学I の6記事すべてが横スクロールしていた。原因は MathJax の
出力そのもので、記事HTMLの書き方ではない。直したのは `css/lesson-theme.css` の共通CSS。

## 84.1 ★ インラインの `mjx-container` は `display:inline` なので溢れが止まらない
`overflow-x` も `max-width` も**インライン要素には効かない**。`inline-block` にすれば
効くようになるが、今度は「overflow が visible 以外のインラインブロックは、
ベースラインが下マージン端になる」というCSSの規則で、数式が行から沈む。
`vertical-align:bottom` で打ち消す。この3つは**セットで入れないと必ずどれかが壊れる**。
...
.box,.drill,.sol,.attempt,.hint,.step{min-width:0}
```

`min-width:0` が要る理由：flex / grid の子は既定で `min-width:auto`（＝中身より縮まない）
なので、親が縮んでも数式の箱が縮まず、はみ出しが外へ伝わる。

## 84.2 ★ ブロック数式に `max-width:100%` を付けてはいけない
記事側で負のマージンを使って本文より広く取っている箇所があり、`max-width:100%` を
足すと 842px → 818px に縮んでしまった。付けない。

## 84.3 ★ `padding-bottom` はデスクトップで文書を伸ばす
スクロールバーの分を空けるつもりで `padding-bottom:5px` を全幅に入れたら、
1440px の文書が 100〜175px 伸びた（1440px では横スクロールする数式が1本も無いのに、
全部の数式に余白が付くため）。`@media (max-width:1024px)` に閉じ込める。

## 84.4 2行のブロック数式は `aligned` でまとめる
狭い画面では、溢れた行は左寄せでスクロールし、短い行は中央寄せのまま残るので、
2行の対がずれる（4-1 で55px、2-2 で74.9px）。`\begin{aligned}` に入れると
1つの箱になり、ずれない。MathJax v3 tex-svg で動作確認済み（エラー0）。

## 84.5 測り方の手引き
はみ出し要素を数えるとき、**fixed の先祖を持つもの**（浮遊ナビ）と
**スクロール容器の中のもの**は除く。ただし `body` / `html` 自身は除かない
（除くと候補が0件になる）。この切り分けに3回失敗した。

## 84.6 State
`4f0ec2b`（CSS、r66）、`4c41747`（インライン6本をブロックへ、r67）、
`f59e65c`（2本を `aligned` へ、r68）。

---

# 85 第3回に主要衛星を足す（木星の4枚）

## 85.1 衛星カードは第2回の岩石惑星カードと同じ作り
巨大惑星が［断面］＋［球］なのに対し、衛星は**［地図］＋［球］**（`mod.map-view.has-globe`、
`data-grid="equi"`）。第2回の機械（`mvGrid` / `mvMap` / `mvSide` / `mvSpread` / `mvGeoList`）を
そのまま使うので、カード側は `data-x` / `data-y` / `data-lat` / `data-lon` を持つだけでよい。
並びは軌道の順で、母惑星のカードの直後に置く。

## 85.2 ★ USGS のモザイクは製品ごとに左端の経度が違う
4枚とも「Simple Cylindrical・惑星測地系・**+West** 0〜360」だが、配布画像の**定義域が違う**。

| 衛星 | 配布画像の定義域 | 左端 | このサイトでの処理 |
|---|---|---|---|
| イオ | −180〜180 | 東経180度 | そのまま |
| エウロパ／ガニメデ／カリスト | 0〜360 | 東経0度 | 横に512px（半周）ずらす |

どれも**東経は右へ増える**。メタデータを読むだけでは決まらないので、
Gazetteer の座標を重ねて実際の地形と合うかを見る（§53.2 と同じ手順）。
カリストはヴァルハラの同心円が決め手になった（左端を東経0度と仮定した版では
何も無い所に落ちる）。エウロパはプウィルを拡大して光条クレーターを確認。

## 85.3 座標は東経に直して表に載せる
Gazetteer は木星の衛星を西経で記録している。表と地図は**東経**（＝360−西経）に統一し、
その旨を表の下に注記した。両方を併記すると数字が増えすぎて覚えられない。

## 85.4 欠損は figcaption に率で書く。埋めない
| 衛星 | 未撮影（黒） |
|---|---|
| イオ | なし（0.00%） |
| エウロパ | 南緯83.5度より南（南緯75度以南の約49%、全球の約4.1%） |
| ガニメデ | 両極（北緯75度以北 約29%、南緯75度以南 約15%、全球 約3.6%） |
| カリスト | 両極（南緯75度以南 約32%、北緯75度以北 約7%、全球 約3.8%） |

§82.3 と同じで、埋める加工はしない。ずらした場合も「ずらした以外の加工はしていない」と書く。

## 85.5 出さなかった数値
- **ボーサウレ山の高さ** — 研究によって値の幅が大きいので数値を出さず、確認先だけ書いた
- **ジュノーの現況** — 延長ミッションの期限は2025年9月。NASA のページ（2026年7月4日更新）は
  「運用中」と表示しているが、その後の延長の有無は確認できなかったので、そう書いた

## 85.6 目次で衛星を惑星の下の階層にする
`.toc` の惑星 `<li>` の中に `<ul class="sub">` を入れ子にする。衛星は**番号を持たない**。
- CSS：字下げ16px、11.5px、`--ink-faint`、左に1pxのヘアライン（どの惑星の下かを線で示す）
- 番号は**惑星の通し番号**に戻す（03 木星／04 土星／05 天王星／06 海王星／07 以降は横断の節）
- 本文の見出しは `sec-head is-sub` で番号なし。ただし**見出しの文字の左端は惑星と揃える**
  （`padding-left:51.5px` ＝ 番号の幅37.5 ＋ gap 14。560px以下は20px）。
  番号を外しただけだと、下の階層なのに見出しが外側へ出てしまう
- 追従：現在地が `.toc .sub` の中にあるとき、親 `<li>` の先頭リンクに `.parent` を付ける。
  丸い下地（`.active`）とは競合させず、文字と番号の色だけ上げる

## 85.7 ★ 第1回のインラインスクリプトから2つ落ちていた
第2回を作るときに写し漏れ、第3回もそれを引き継いでいた。

| ハンドラ | 1-1 | 1-2 | 1-3 |
|---|---|---|---|
| 目次の引き出し（`nav-open` の付け外し） | ○ | **✗→復帰** | **✗→復帰** |
| 印刷時に `<details>` を開く（`beforeprint`） | ○ | **✗→復帰** | **✗→復帰** |

- 引き出しが無いと、900px以下で「目次」ボタンが**何も起きない**（レールは `translateX(-100%)` のまま）
- `@media print` のコメントが「下の beforeprint で open 属性を足している」と書いているのに、
  その `beforeprint` 自体が無い状態だった。**コメントは実装の存在を保証しない**

## 85.8 インラインスクリプトの重複（共通化の判断材料）
1-2 と 1-3 のインラインスクリプトは 489行 / 518行で、**差は35行だけ**（約93%が同一）。
違いは ① 目次の親ハイライト（第3回で追加） ② 環の図の開閉（第3回のみ）の2つ。
`mvNext` `mvPlace` `mvGrid` `mvMap` `mvSide` `mvSpread` `mvGeoList` `mvLinks` `mvKeepTop`
`visibleStats` `mvNarrow` `mvSvg` `onScroll` が3ファイルに別々に置かれている。

**共通化すれば §85.7 の写し漏れは起きなくなる**（1箇所しか無いので落としようがない）。
一方で、`js/` に出すとキャッシュバスターの対象になり、1回分の修正で全ページの `?v=` が
上がる。回ごとに機械が育っている段階（第2回で地図、第3回で断面・環）なので、
**いまは各ファイルに置いたままにし、第4回を作るときに `js/memoryverse.js` へ切り出す**。
それまでは「新しい回を作るときは前の回と `diff` を取る」で代用する。

## 85.9 State
`a084ddb`（第2回のハンドラ復帰）、`cf4efa3`（木星の衛星4枚・目次の階層・DESIGN §84/§85）。

---

# 86 第3回に残り4枚（エンケラドゥス・タイタン・ミランダ・トリトン）

## 86.1 ★ USGS だけを見て「全球図が無い」と判断しかけた
最初、USGS Astrogeology だけを探して「タイタンは緯度±55度まで」「トリトンはメルカトル」
「ミランダは極平射だけ」と報告した。**NASA Photojournal に全球の正距円筒版があった**。

| 衛星 | 採用 | 投影・左端 | 欠損 |
|---|---|---|---|
| エンケラドゥス | PIA18435（カッシーニ ISS、原寸 15960×7980） | 正距円筒・左端＝東経0度 → 半周ずらす | 0.35% |
| タイタン | PIA19658（カッシーニ ISS 938nm、4km/画素） | 正距円筒・左端＝東経0度 → 半周ずらす | 0.64% |
| トリトン | PIA18668（ボイジャー2号／Schenk・LPI、600m/画素） | 正距円筒・**左端＝東経180度**（ずらし不要） | **38.5%** |
| ミランダ | PIA01490（ボイジャー2号、9枚の合成） | **南極中心の正射投影の円盤** | 北半球は未撮影 |

USGS の "Near Global"（±55度）と Photojournal の全球版は**別の製品**。
検索は1つの配布元で打ち切らないこと。

## 86.2 ★ ミランダは「幾何が書いてある製品」と「読める製品」が別だった
- **PIA02217** は説明に「controlled mosaic を南極中心の正射投影に変換した」と明記されているが、
  595×595・低コントラストで、コロナ3つを目で区別できない。グリッドを引いても
  何を指しているのか読めない
- **PIA01490** は 1581×1600 で読めるが、投影の記述が無い

そこで **USGS の地名図（`miranda_nomenclature`、南極中心の極平射）を「どの地形がどれか」の
真値として使い**、PIA01490 に当てはめて向きを決めた。地名図は上が0度・左が270度、
つまり**方位（真上から時計回り）＝東経**。これを PIA01490 に移すと、

`方位 = 東経 + 89.3度`、`中心からの距離 = R·cos(緯度)`、`R = 幅の0.4717`

で、**ヴェローナ・ルペス（リムの明るい崖）・アーデン・コロナ（同心の帯）・
インヴァネス・コロナ（台形）・エルシノア・コロナ・マントヴァ領域（古い地形）の5つが同時に合う**。
円盤の中心と半径はリムに最小二乗で円を当てた実測値。**89.3度は公表値ではなく実測**なので、
figcaption にその旨を書いた。

★ 途中、PIA01490 を素直に「南極中心の正射投影」と仮定して2地形から解こうとして失敗した。
ボイジャー2号の副衛星点は極ではないので、**2地形では手がかりが足りない**。
第三の製品（地名図）を真値に使うまで解けなかった。

## 86.3 円盤の地図では、ラベルを矩形ではなく円の内側へ寄せる
ヴェローナ・ルペスは `r = 0.95R` でリムのすぐ内側にあり、矩形クランプだけでは
ラベルが黒い余白へ出る。`data-grid="spolar"` のときだけ、ラベルの**四隅まで**円に収まるよう
中心方向へ引き戻す（`mvMap`）。狭い画面（番号の丸）は対象外。

## 86.4 spolar グリッド
緯度＝同心円（0/−30/−60度）、経度＝放射線（30度ごと、狭い画面は60度ごと）。
★ 緯度のラベルは**経度のラベルと45度ずらした方角**に置く。真上に置くと、
`az0` のせいで真上に来る経度ラベル（270度）と重なった。

## 86.5 ★ トリトンの未撮影域は「黒いまま」にしない
全球の38.5%（北緯45度より北は100%）が未撮影で、黒のままだと**描画の失敗に見える**。
未撮影の画素に薄い斜線（`(x+y)%10<2`、背景 rgb(20,22,28)／線 rgb(48,52,64)）を重ねた。
**これは加工なので figcaption に明記**。地図と球で同じ画像を使うので、球でも斜線が出る。
球の注記にも「データが無いということで、描画の失敗ではありません」と書いた。

## 86.6 ★ 焼き込みの枠を切り出すと、にじみが継ぎ目になる
タイタンの PIA19658 はタイトル・目盛・スケールバーが焼き込まれている。青い枠線
（行205-207/2251-2253、列183-184/4253-4256）を検出して内側 4068×2043（＝1.991:1）を
切り出したが、**枠の JPEG のにじみが端14画素に残り、半周ずらしたあと図の中央に
黒い縦筋として現れた**。左右それぞれ14画素（幅の0.3%）をすぐ内側の列で置き換えて消し、
figcaption に明記。経度の対応はずらしていない。

## 86.7 座標系が衛星ごとに違う
| 衛星 | Gazetteer の座標系 |
|---|---|
| イオ／エウロパ／ガニメデ／カリスト／エンケラドゥス／タイタン | 惑星測地系・**+West** 0〜360 |
| トリトン | 惑星測地系・**+East** 0〜360 |
| ミランダ | **惑星中心系**・+East 0〜360 |

表と地図は全部**東経**に統一し、どの系から直したかを表の下に書いた。

## 86.8 ミランダに［球］を出さない
全球のテクスチャが無い、という以前に、**南極中心の円盤を球に貼れば間違った絵になる**
（不完全ではなく誤り）。カードに「この衛星に［球］はありません」の枠を置いて理由を書いた。
`.mod` から `has-globe` を外すだけで、球の IIFE（`.mod.has-globe` を見る）が素通りする。

## 86.9 出さなかった数値
- **ヴェローナ・ルペスの高さ** — 研究による幅が大きいので数値を出さず、確認先だけ
- **ドラゴンフライの打ち上げ・到着** — 計画の日程は変わるので「覚えないでください」と添えた
- **ホイヘンスの着陸地点の座標** — 出典によって東経・西経の扱いが揺れるので、
  「明るいアディリと暗いシャングリラの境目、南緯約10度」と言葉で書き、確認先を示した

## 86.10 なぜ切り替えが無いのかを、画面で言う（§87 へ）
8枚のうちミランダだけ［地図］［球］の切り替えが無い。仕様どおりだが、**画面上では
作り忘れと見分けがつかない**。トグルがあるはずの位置に、押せない1行を置いた。

```html
<p class="vsw-note">球はありません。ミランダは南半球しか撮影されておらず、全球の地図が作れないためです</p>
```

- `<p>`（ボタンではない）。枠も下地も付けない
- 高さは `.vsw` と同じ 29px（`min-height`）、`margin` も同じ。カード上端からの位置は
  他の7枚と同じ 16px
- 色は `--ink-faint`。figcaption と同じ
- 「見え方」にも1文足した（北半球は誰も見たことがない／この回で唯一、展開図も球も作れない）

★ サイト共通の `p{max-width:62ch}` と `body.is-article main p{margin:10px 0;color:var(--ink-soft)}`
に負けて、幅430pxで折り返し・余白も色も別物になっていた。**クラス2つ
（`.mod-b .vsw-note`）まで specificity を上げ、`max-width:none` を明示**して直した。

★ ついでに分かったこと：同じ理由で **`.globe-note` と `.globe-cap` と `.ring-narrow` は
意図した `--ink-faint`(0.46) ではなく `--ink-soft`(0.68) で表示されている**（全11枚）。
`figcaption` は `<p>` ではないので影響を受けず 0.46。**カード内の注記の濃さが
2種類に割れている**。見た目の実害は小さいので今回は直していないが、第4回で
`js/memoryverse.js` を切り出すとき（§85.8）に併せて整理する。

## 86.11 「作れないもの」の点検（12枚）
| | 断面/地図 | 球 | 球のピン | 環の図 |
|---|---|---|---|---|
| 木星・土星・天王星・海王星 | ○ | ○ | **無し**（globe-note で理由を明記。大赤斑は経度が動く／天王星・海王星は模様が無い） | ○ |
| イオ〜トリトン（7枚） | ○ | ○ | ○ | — |
| ミランダ | ○ | **無し**（§86.10） | — | — |

★ 巨大惑星の「地形を開く」ボタンは、ピンが0件のとき JS が `bOpen.hidden=true` にしており、
`display:none` になっていることを実測で確認した（押せる死んだボタンは残っていない）。
一度「死んでいる」と誤判定したが、原因は検査スクリプトの側（`offsetParent!==null||true`
と、hidden 要素への JS `.click()`）だった。

## 86.12 State
`c335537`（残り4枚）、`974a8a5`（ミランダの1行）。

---

# 87 第3回の残りの節（到達目標・見方・探査・訓練・演習・付録）

## 87.1 §02 は「断面で覚える」から「この回の見方」へ
カードが2種類（断面／地図）になったので、節の役割も変わった。先頭に**分け方の表**を置く。
...
- 球にピンを置く／置かないの違いを、**衛星側（地形は動かない）と巨大惑星側（大赤斑は
  経度が動く／天王星・海王星は模様が無い）を並べて**書いた
- 第2回の「展開図で覚えて球で確かめる」が衛星でそのまま使えることを、4手順で明示
- **「作れないものは作れないと書いてあります」**の枠を新設。ミランダの球、トリトンの
  38.5%、出さなかった数値（ボーサウレ山・ヴェローナ・ルペスの高さ、ジュノーの現況）を
  1か所に集め、**「知らない」と「まだ誰も見ていない」を区別できることが隠れた到達目標**だと書いた

## 87.2 演習5 が、この回の主題
「分かっていないことを挙げる」で、エウロパの海／トリトンの北半球／ミランダの成因を
**2種類に分ける**ことを求める。

- **まだ確かめられていない**（エウロパ、ミランダ）——手がかりはあるが決め手にならない。
  もっと良い観測か新しい考え方が要る
- **まだ誰も見ていない**（トリトン北半球）——手がかりがそもそも無い。行けば分かる

同じ「未確定」でも**次に何をすれば進むかが違う**、という点を答えに書いた。

## 87.3 探査の歴史は「何回行ったか」で読ませる
表は第2回§10と同じ4列（探査機／行き先／年／明らかにしたこと）。指定の7機に
**ニューホライズンズ**を足した（イオのカードがトバシュタルの噴煙で引用しているため）。

- ★ **ジュノーの現況は確認できないまま**なので、その旨を枠で書いた（延長期限2025年9月、
  NASA ページは2026年7月4日更新で "Active Mission" 表示、その後の延長は不明）
- ジュース・エウロパ・クリッパーは「**まだ着いていない**」状態のほうを覚えさせる
- ボイジャー2号が天王星・海王星に行った唯一の探査機であること、ミランダ（1986-01-24）と
  トリトン（1989-08-25）の図が一度きりの通過で得られたものであることを、最後の枠で
  カードの作りに直結させた

## 87.4 訓練メニューは3週間だが、4週目を想定して書く
12枚は第2回（5枚）の倍以上ある。「3週間で終わらなくても構いません。**4週目に入る人の
ほうが多いはずです**」と明記した。効果を誇張しないための書き方でもある。

## 87.5 付録B に弱を3つ足した
断面を重ねる（既出）に加えて、**衛星を母惑星の直後に軌道順で置く／展開図が作れない
天体を円盤のまま扱う／未撮影の範囲に斜線を重ねる**の3つを「弱（検証した研究は
確認できていない設計判断）」として足した。この回で新しく作った手順はこれで全部。

## 87.6 ★ 分量が第2回の2.1倍になった
本文の文字数（タグ・SVG を除く）：第1回 10,220／第2回 17,193／**第3回 36,946**。
カード12枚で約24,000字を占める。**1枚あたりの文量は第2回と同程度**なので、
長さは枚数から来ている。削るなら節ではなくカードの重複表現から。

残っている重複（今回は残した）：
- 「自転と公転が同じで、つねに同じ面を〜へ向けています」が**8回**（衛星の基本行と
  球の注記で二重）。§02 に一度書いて各カードから外せる
- ミランダに球が無い理由が**4か所**（トグル位置の1行／カード内の枠／見え方／§02）
- トリトンの38.5%が**4か所**（figcaption／球の注記／§02／§09）

## 87.7 同じ文の8回の繰り返しを §02 に集約
「自転と公転が同じで、つねに同じ面を〜へ向けています」が、衛星8枚の「基本」行と
球の注記で**計15回**出ていた（基本行8＋球の注記7。ミランダは球が無いので7）。
§02 に1つの枠として集約し、**「以降のカードでは繰り返しません」**と明記して全部落とした。

★ 集約するときに、ついでに書こうとした「既定の向きで正面に来るのが母惑星を向いた面
とは限らない」は**逆だった**。同期回転する衛星の本初子午線は、IAU の決まりで
**母惑星を向く子午線**に取る。つまり「基準に戻す」で正面に来る面が、そのまま
母惑星から見えている面。USGS のミランダ地名図が、0度の向きに
「URANUS-FACING HEMISPHERE」と印刷しているのが直接の証拠。
**重複を減らす作業のついでに新しい事実を足すと、検算が甘くなる。**

残した重複（意図的）——ミランダの球が無い理由4か所、トリトンの38.5%が4か所。
...
# 89 第4回 小天体（配置図と最初の2枚）

## 89.1 ★ 分け方が「地面の有無」では足りない
第2回＝地図、第3回＝断面、という分け方は第4回で破綻する。探査機が行っていない
ものが多く、しかも数が多すぎて1つずつカードにできない（小惑星だけで150万個超）。
**対象ごとに手がかりを変える**ことにし、§02 に表で明記した。

| 種類 | 対象 | 作り | 覚えるもの |
|---|---|---|---|
| 地図が作れる | ケレス・ベスタ・冥王星・カロン | ［地図］＋［球］ | 地形の位置 |
| 形が特徴 | リュウグウ・ベンヌ・エロスほか | **実画像のみ** | 形と、どこにあるか |
| 領域 | 小惑星帯・カイパーベルト・散乱円盤・オールトの雲 | **配置図** | どこに何が広がっているか |

## 89.2 配置図（この回の背骨）
対数目盛1本に 0.3 AU 〜 40万 AU を収める。★ 当初の指定は「1 AU から」だったが、
**それだと水星(0.387)と金星(0.723)が図の外へ出る**。8惑星を残すほうが優先なので
左端を 0.3 AU にした。

★ **対数目盛では「オールトの雲が恒星までの何割か」が読めない。**10万/27万＝37% が、
対数では93%の位置に見えてしまう。そこで図の下に**同じ距離の線形バー**を1本足した。
2つの目盛を並べることで、「対数は近くを引き伸ばす」ことも同時に伝わる。

★ 惑星のガイド線を軸まで下ろしたら、小惑星帯とカイパーベルトのラベルを貫いた
（ラベルがちょうど惑星の列の上に来る）。**線は点の上下の短い目印だけにした。**

## 89.3 領域のカードの作り（小惑星帯）
地図も球も無いので、`.mod`（追加クラスなし）に `.vsw-note`（§87.1 で作った
「切り替えが無い理由を書く1行」）を置き、図＋`.row` の並びだけで組んだ。
放射状配置（第1回）には `.stat[data-ang]` が要るので、`.stat` を持たない
このカードは JS に触られない。**`.mod-b .row` は `.open` に依存しないので、
開閉の無いカードでも中身が出る。**

★ **メモリーバースには MathJax が無い。**カークウッドの空隙の式を TeX で書いたら
生のまま表示された。`a ＝ a<sub>J</sub> ×（q ÷ p）<sup>2/3</sup>` のように
素の HTML で書く。この回に数式を持ち込まないこと。
**→ §94 で改めた。メモリーバース全体で MathJax を読み込み、式は TeX で書く。**

## 89.4 「密集している」という誤解を崩す
- NASA の記述「すべての小惑星の質量を合わせても地球の月より小さい」を引く
- JPL の GM から計算して、**ケレス1つで月の約1.3%、上位4つで約2%**
- ★ **隣の小惑星まで平均約360万km は本教材の概算**。帯を内径2.1・外径3.3・厚さ1 AU の
  ドーナツ（約 6.8×10²⁵ km³）と見て157万個で割った立方根。厚さの取り方で2倍動くので、
  **桁（百万km台）だけ覚えてくださいと明記**した

## 89.5 カークウッドの空隙は、第3回のラプラス共鳴と地続き
同じ「繰り返し同じ配置で引き合う」が、第3回では**閉じ込めて潮汐加熱を生み**、
ここでは**軌道を歪めて弾き出す**。トロヤ群（1:1）は閉じ込められた側。
**共鳴＝軌道が揃うと効果が積み重なる、とだけ覚え、結果は場合ごとに見る**と書いた。

## 89.6 ケレスの画像と較正
USGS Astrogeology「Ceres Dawn FC Global Mosaic 140m」（配布 JPEG 1024×512）。
**Simple Cylindrical・惑星中心系・Positive East・定義域 −180〜180 ＝ 左端が東経180度**で、
このサイトの既定と同じ。**ずらし不要**。Gazetteer（Ceres はターゲット 148、
惑星中心系・東経0〜360度）の座標を重ね、オッカトルが中央に白い斑点のあるクレーターに、
ハウラニが光条クレーターに、アフナ山が孤立した高まりに載ることを確認した。
USGS の Ceres Nomenclature 図（左端180度・Positive East と図中に明記）とも一致。
欠損 5.65%（南緯75度以南で52%）。埋める加工なし。

★ Gazetteer のターゲット ID は連番ではない。148 は総当たりでは見つからず、
**AdvancedSearch ページの `<option value="148_Ceres">` から引いた**。
...
# 90 第4回 ベスタ〜トロヤ群、図の横送り

## 90.1 ★ 図は縮めずに横へ送る（第1〜4回の8枚）
viewBox 640 の図を 390px 幅に縮めると、9.5 の注記が実寸 4.6px になる（第1回2枚・第2回2枚・
第3回の横断2枚・第4回2枚、すべて 4.4〜4.9px。実測）。縦長の別図は作らない——
**同じ図を2つの形で覚えることになり、背骨の図として具合が悪い**。

- `<figure class="fig">` 直下の `<svg>` を `<div class="fig-scroll">` で包む。svg は `min-width:640px`
- 本文の枠が 640px を割ったときだけ送れる（768px 以上は従来どおり）
- 合図は3つ：上に1行「横に送ると続きが見られます →」（`.fig.is-scrollable::before`）、
  送れる側の端のフェード（mask-image）、細いスクロールバー。**iOS はスクロールバーを
  動かすまで出さない**ので、1行とフェードが主な合図
- 状態は `js/memoryverse.js` 末尾の小さな関数がクラスにするだけ（`.is-scrollable` / `.is-start` / `.is-end`）
- ページ全体の横スクロールは 0（360/390/768/1440px × 4ファイル。`.fig-scroll` 以外で
  右端が画面を超える要素を数えて確認）

★ **対象外にしたもの**：カードの中の環の図（`.ring-svg`、第3回4枚）と断面（`.cut-svg`）。
どちらも狭い画面ではラベルを畳んで一覧に落とす別の扱いが既にある（§54.5／§81）。
ただし環の図は、畳んだあとに残る題・軸の数字がやはり 4.6px になる。揃えるなら
`.ring-btn` の中を `.fig-scroll` にして `.ring-narrow` をやめる形になる——未着手。
**★ 保留（指示あり）：第3回のカードの作りが変わるので、いまは手を付けない。
第4回が終わってから、まとめて判断する。**

## 90.2 配置図の2点
- 線形バーの「プロキシマ」をバーの上（菱形の真上）へ。37% の文と続けて読めていた
- 対数軸の左端「1 AU」を「1」に。単位は右端の「AU」1つだけ
- ★ **地球近傍小惑星の目印を足した。**「配置図のどこか」から入る形なので、図に無い対象が
  あるとその入口だけ形が崩れる。範囲を持つ領域ではなく「地球の軌道の近くを通るもの」の
  呼び名なので、**帯や箱ではなく、軸の上から 1.15 AU を指す短い矢印**（琥珀色＝小惑星帯の色。
  出どころが小惑星帯であることを色でつなぐ）。地球の点と重ならないよう、惑星の列ではなく軸の直上
- 図の下の説明と数値表に1行ずつ：もとは小惑星帯にいて、木星との共鳴などで軌道を歪められ
  内側へ入ってきた（NASA CNEOS／Granvik ほか 2017, A&A）＝**カークウッドの空隙の
  「共鳴は追い出す」の、追い出された先**。リュウグウ・ベンヌのカードにも同じ1行

## 90.3 ★ 形が特徴の天体は「同じ縮尺で並べる」（リュウグウとベンヌ）
地図でも球でもない、第4回で新しいカードの形。`.pair-fig` に実画像2枚を横に並べる。
- **縮尺を揃えるのが要点。**2枚とも「本体の幅＝画像の幅÷1.08」（余白は本体幅の4%ずつ）で
  切り出し、CSS の幅の比を赤道の差し渡しの比（1,004m：505m、JPL SBDB）にした。
  500m の物差しも同じ計算。実測で m/px が3つとも 0.2% 以内で一致（1440px・390px）
- ★ 物差しの `%` は `.pair-scale`（段の 91.2%）に対する値。最初は段に対する値を
  そのまま入れて、**約1割短く出た**
- ★ **明るさは比べさせない。**2枚とも明るさを持ち上げた画像で、度合いが違う（実際は
  どちらも反射率4〜5%）。figcaption に明記
- 比べる表（`.pair-table`）は ★ **横送りにしない**。最初は最小幅 520px を付けたら、
  390px でベンヌの列が画面外に出て1列ずつしか見えなかった。狭い画面は文字と余白を詰めて3列を収める
- ★ 900px 以下で `.row` が縦並びになると、`align-items:flex-start` のせいで `.v` が中身の幅まで
  広がり、行ごとはみ出した（390px で右端 561px）。`.mod-b .row > .v{align-self:stretch}`

画像：リュウグウは Wikimedia Commons「Ryugu colored desaturated.png」（JAXA/ISAS DARTS の
ONC-T 2018-07-12 を Meli thev・Nrco0e が処理、CC BY 4.0）を切り抜き。ベンヌは
「Bennu mosaic OSIRIS-REx (square).png」（NASA/Goddard/University of Arizona）を切り抜き・縮小。
**改変した旨を figcaption と記事末に書いた**（CC BY の要件）。

## 90.4 試料から分かったこと——3つに分けて置く
更新が続く分野なので、各項目に発表元と日付、節の頭に「2026年9月25日時点」と書いた。
第3回の演習5の2分類に、**「分かっていること」**を足した3つの箱（`.known`）。
- 分かっていること：生命の材料になる分子が生命と無関係にできていた／**生命の跡ではない**
  （アミノ酸の左右がほぼ半々。NASA が「生命の証拠ではない」と明記）
- まだ確かめられていない：材料が地球に届いて生命になったか／なぜ左手型だけか
- まだ誰も見ていない：**ベンヌの試料の70%以上**（将来のために封を切らず保管、NASA 2024-02-15）／
  **リュウグウの糖**（量が足りず調べていない。Furukawa 2025-12-18）
- ★ **「生命の証拠がない」は「まだ分からない」ではない**——調べた結果として分かっている側に置いた

★ 検算で落とした・直したもの：リュウグウのアミノ酸は「約20種」ではなく **15種**（Naraoka 2023）。
リュウグウの5種類の核酸塩基は Oba 2023（ウラシルのみ）ではなく **Koga 2026**。
リュウグウの糖は「見つからなかった」ではなく「調べていない」。2026年の論文は大学・報道の解説で
確認し、本文は読んでいない旨を本文に書いた。

## 90.5 ベスタ
USGS「Vesta Dawn FC HAMO Global Mosaic 60m」（配布 JPEG 1024×512、無加工）。
Simple Cylindrical・惑星中心系・Positive East・定義域 −180〜180 で、**ケレスと同じくずらし不要**。
★ 経度は IAU の Claudia Double Prime 系（クラウディアが東経146度）。**ドーン初期の論文の
Dawn-Claudia 系とは150度ずれる**。Gazetteer（ターゲット 144）も同じ系なので、そのまま重なる。
雪だるま・レアシルビア・ディヴァリア溝が載ることを目視で確認。北緯60度以北に影の欠け。

★ 中身は書き方に注意。NASA の解説は「地殻・マントル・核に分化」だが、**2025年の再解析
（Park ほか, JPL 2025-05-14）は「核はとても小さいか、無いかもしれない」**。
「表面が溶けた（HED 隕石の玄武岩）は確か、核の有無はまだ確かめられていない」と分けた。
小惑星帯の表の「表面が溶けて分化しており」もこれに合わせて直した。

## 90.6 木星のトロヤ群（領域のカード）
北極の側から見た模式図（本教材で作成、生成は Python で座標を計算）。太陽・木星・L4/L5 の
正三角形、60度、公転の向き、小惑星帯を同じ縮尺で。L4 の点を多めに描いた（実際に約1.45倍）。
- 数は MPC の一覧（2026-05-30 更新）を数えた 15,765個（L4 9,823／L5 5,942）。「100万個」は
  NASA の出典が無いので使わず、「小惑星帯全体に匹敵」（NASA）とした
- **小惑星帯のカードの「共鳴は閉じ込めも追い出しもする」を、ここで閉じ込める側として回収**
- どこから来たか＝まだ確かめられていない（2012年の WISE は別の説に傾く）。
  探査＝まだ誰も見ていない（ルーシーの最初のトロヤ群到着は2027年8月）。演習5の2分類を両方使う
- ルーシーの日付は NASA と SwRI で1日ずれるもの（時差）があるので月までにした
...
# 91 第4回 カイパーベルト、冥王星とカロン

## 91.1 カイパーベルト（領域のカード）——分布は模式ではなく実データ
IAU小惑星センターの TNO 一覧（2026-09-25 更新、4,103行。うち2行は q・Q の列がくっついて
いて落ちるので、`epoch M Peri Node Incl e a` の並びを右から拾う正規表現で読んだ）の
軌道長半径を 0.5 AU ごとに数えた柱状図。**共鳴の山が実データにそのまま出る**
（36.4＝4:3、39.4＝3:2、47.7＝2:1、55.4＝5:2。海王星 30.07 AU からカークウッドと同じ式で計算）。
- ★ **小惑星帯では共鳴の距離が空き、ここでは溜まる。**同じ式・逆の結果として、
  「共鳴は閉じ込めも追い出しもする」の3つめの例にした（空隙→トロヤ群→プルーティーノ）
- ★ 見つけやすさの偏りを補正していないので、**山の高さは比べさせない、位置だけ読む**と図と説明に明記
- 外縁 47±1 AU（Trujillo・Brown 2001 の補正済みの値）。「カイパーの崖」は通称として添えるだけ
- 「1兆個の彗星」は NASA の現行ページに無い（2009年の資料のみ）ので使わない

## 91.2 ★ 冥王星とカロンは「同じ高さに並べる」
第3回は衛星を惑星の下の階層に置いた（目次の字下げ）。カロンだけ扱いを変える理由を本文に書き、
節を「08 冥王星とカロン」にして、その下に冥王星とカロンを**兄弟として**並べた。
地図の前に、関係を見せる箱（`.box.teach`）と共通重心の図を置く。
- 図：冥王星とカロン／地球と月を、**天体どうしの距離を同じ長さにそろえて**上下に並べる。
  重心の×と「主星が重心のまわりに描く円」。冥王星の円は冥王星の外に出る
- 重心は本教材で計算：19,596 × 106.1 ÷ 975.5 ＝ 2,131km（冥王星の半径の1.79倍）、
  地球と月は 384,400 ÷ 82.3 ＝ 4,671km（地球の半径の0.73倍）
- ★ **最初「重さの比の10倍の差が重心を外へ押し出す」と書いたのは不正確だった。**
  距離が主星の半径の何倍か（冥王星 16.5、地球 60.3）も同じくらい効く。2つ重なっていると直した
- 互いに同じ面を向け合う（公転＝両方の自転＝6.387日）、小さな4衛星も共通重心を回る、を箇条で
- 分類の話はカードに入れず、1行で「変わったのは定義、冥王星そのものは変わっていない、
  後の独立した枠で」とだけ予告した

## 91.3 ★ 冥王星の地図だけ中央が東経180度（`data-lonc` / `data-home`）
USGS「Pluto New Horizons LORRI MVIC Global Mosaic 300m」は**左端0°・中央180°・右端360°**。
このサイトの既定（左端180°）に回すと、覚える中心のスプートニク平原（東経179度）が
**左右の端で割れる**。画像は回さず、`js/memoryverse.js` に属性を2つ足した。
- `data-lonc="180"`：地図の中央の経度。グリッドの経度ラベルと、球のテクスチャのずらし
  （`tex.wrapS=RepeatWrapping; tex.offset.x=-lonc/360`）に使う
- `data-home="180"`：球の既定の向き（正面に出す経度）。「基準に戻す」もここへ戻る
- **属性が無いときの計算は従来と同じ**（c0=0、H0=0）。第2〜4回の球 20個すべてで表示・
  地形リストの件数を確認し、ケレスの球の既定の向きが変わらないことも画像で確認
- 冥王星の球は既定でハートが正面、ピン1がハートの左半分に載ることを画像で確認

カロンの配布画像は左端 −180°で既定どおり（USGS の製品ページの「0 to 360」表記は誤りで、
ラベルと画像はどちらも −180〜180）。Gazetteer は両天体とも 0〜360°E なので、カロンは読み替える。

★ 名前の更新：「クトゥルフ」→ **ベルトン地域**（2023-09-22 承認）、カロンの「モルドール」→
**ネバーランド地域**（2026-02-02 承認）。どちらも旧称を括弧で添えた。セレニティ谷・
ヴァルカン平原は非公式で Gazetteer に座標が無いので、地図には置かない。
未撮影：冥王星 約25%、カロン 約27%（面積で重み付け。南半球は冬の闇）。

## 91.4 ラベルの向き（`data-side`）
ハートのまわりに4つ集まるので、スプートニク平原=up、ベルトン=left、ライト山=down、
タルタロス=right。カロンはマンジェト=left、アルゴ=right、キューブリック=down。
ベスタのベネネイアも right にした（1024px 以下でディヴァリア溝と重なっていた）。
1440/1024/768px で重なり0。
★ **561〜700px 付近では、第2回・第3回・ケレスの地図にも既に重なりがある**
（月・火星・イオ・カリスト・タイタンなど計20組超、実測）。ラベルが番号に畳まれるのは
560px 以下なので、その少し上の幅が抜けている。第4回とは別件——未着手。
...
# 92 第4回 散乱円盤・準惑星の節・彗星・オールトの雲（カードの完成）

## 92.1 散乱円盤（領域のカード）——近日点×軌道長半径の散布図も実データ
MPC の TNO 一覧と Centaurs/SDO 一覧（どちらも 2026-09-25）から 5,926 天体を描いた。
「近日点は海王星のそば（30〜40 AU）のまま、軌道だけが右へ伸びる」が形でそのまま見える。
セドナは両一覧に無いので JPL SBDB の値で別に置いた。
★ **点を `<circle>` で 5,900 個並べたら SVG だけで 413KB になった。**1本の `<path>` に
「`m dx dy h0`＋`stroke-linecap:round`」で打ち、座標を 0.5 単位に丸めて重複を除くと
4,108 点・**37KB**。見た目は同じ。
エリス・マケマケ・ハウメアは表1つ。エリスは「大きさでは負け（2,326 対 2,377km）、
重さでは勝つ（1.27倍）」。NASA のエリスのページは直径を「約2,400km」のままなので、
掩蔽の Sicardy 2011 を出典にした。「10番目の惑星」（JPL 2005-07-29）→ 2006年の定義へ。

## 92.2 ★ 準惑星への変更は「節」で、カードにしない（§10）
天体の性質ではなく呼び方の話なので、`.mod` を使わず箱4つで組んだ：
決められたこと（teach）／冥王星そのものは変わっていない（principle）／
議論は続いている（warn）／この教材が優先すること（target）。**演習には出さない**と本文に明記。
- 定義の3条件と準惑星の区分は IAU iau0603 の文言から。決議 6A は 237対157（棄権17）
- ★ 「424人が投票」は報道の数字で IAU の発表に無い。「約400人」とし、424 は報道の数と注記
- 2026年4月の NASA 長官の発言は載せたが、**IAU の見直しの発表は無い**と明記
- ケレスの「惑星→小惑星→準惑星」を並べ、「分類は変わる、天体は変わらない」の2例目にした

## 92.3 彗星（形が特徴のもの）
実画像は ESA/Rosetta/NAVCAM（CC BY-SA IGO 3.0）を 1000px に縮小。**縮小版も同じ条件**と明記。
構造図（SVG、本教材作成）：同じ彗星を軌道の2か所に。近いところでコマ＋2本の尾、遠いところで核だけ。
- ★ ダストの尾は、最初「進む向きの逆」を強くしすぎて横倒しになった。太陽と反対 0.85：
  後ろ 0.55 の向きで曲げると、実際の図（ESA）に近い
- ★ 「尾は後ろにたなびく」は誤解、イオンの尾は進む向きと無関係に太陽と正反対——を図と本文に
- フィラエは「計画どおりにいかなかった探査」として正確に：ガス噴射装置が前夜から不調、
  銛が発射されず、**表面に4回触れて**（「2回跳ねた」ではない、ESA の再構成）崖の陰へ。
  日照 約1.3時間（予定 約6.5時間）、約57時間で最初の観測の約8割、11月15日に休眠、
  2015年6〜7月に数回だけ通信、2016年9月2日に割れ目で発見
- 水の D/H は 2015（3倍超）→ 2024 再解析（地球に近い）→ 2026年9月 再解析（約3倍）と行き来。
  「まだ確かめられていない、しかも結論が行き来している」とした
- 彗星のふるさとを表にして、配置図の青い箱と紫の破線に結んだ

## 92.4 ★ オールトの雲——見えていないものの置き方
図（SVG）：①「なぜあると考えるか」（ほぼ全方向から来る彗星→球殻を推定）
②ボイジャー1号の現在地（JPL Horizons：171.85 AU、3.56 AU/年）と内側の端の推定を同じ線形目盛に。
- ★ **NASA の「内側の端まで約300年」は内側の端 約1,000 AU（2018年の図）の値。**NASA の
  解説ページは 2,000〜5,000 AU で、これだと約510〜1,360年（本教材の計算）。両方を並べ、
  「推定の幅が、着くまでの年数の幅になる」を図の結論にした
- 「まだ誰も見ていない」の最も極端な例として、トリトンの北半球（第3回）・冥王星の南半球（§08）と
  並べた。オールトの雲は「まだ確かめられていない」と「まだ誰も見ていない」の**両方**に入る
- 置き方の約束3つ：破線で描く、値に幅を付ける、「推定」と書く
- 2025年の渦巻き構造は**シミュレーションの予測**と明記
- 図の題はカードの見出しと重ならないよう「推定の根拠と、探査機までの距離」

## 92.5 節の番号
09 散乱円盤とエリス／10 準惑星という区分／11 彗星／12 オールトの雲。「残りのカード」の節は削除。
横断 13・訓練メニュー 14・演習 15（まだ枠だけ）。
...
# 93 第4回 残りの節（到達目標・横断・訓練・演習・付録）

## 93.1 到達目標
①配置図を白紙に描いて置ける ②名前→配置図のどこか＋どういう天体か（地形は4天体だけ）
③共鳴が追い出す場合と閉じ込める場合を例で説明できる。③はこの回の軸なので5例を明記。

## 93.2 §02 に巡り方を足した
配置図を描いてから**内から外へ**。★ ページの順とは1か所だけ違う（リュウグウとベンヌは
ページでは小惑星帯のあと、配置図では内側）。巡るときは配置図の順、と書いた。
§02 の表から、実際には扱っていない「エロス」を外し、領域にトロヤ群を足した。

## 93.3 横断（§13）
- 全対象の配置図（§03 と同じ目盛 91.1＋90.1·log10(AU)）。色＝カードの種類。
  セドナ・ボイジャー1号・長周期彗星も足した
- 共鳴の5例の表と、「5つとも同じ1文で説明できる」箱
- 3分類の表。**オールトの雲の行だけ背景色を付け、2列を太字**にして「両方に入る」を表の中で見せた。
  「まだ誰も見ていない」の多くは探査機の通り方で決まる、という読み方を添えた

## 93.4 訓練メニュー
★ **Aの最初の2分を「毎日、配置図を描く」に固定**（指示どおり）。表には書かず、表の下に注記。
第1週5枚（内側）・第2週6枚（外側）・第3週 横断と演習。11枚で第3回（12枚）と同程度。

## 93.5 演習（6本）
演習1は採点法つき：1つ1点・範囲は両端で1点・8惑星はまとめて1点・推定の書き方1点・
線形の横線1点で満点10、目安9。★ 最初「11点」と書いたのは数え違い。
分類の変更は出さない（本文にも明記）。演習6は8項目、オールトの雲が2つに入る。

## 93.6 付録
A：第4回の実際の中身に合わせて改訂。第5回を5項目で具体化（太陽の断面と太陽風／
「押し出されてきたもの」を1本の流れに／配置図の統合／全カード約40枚の巡回／全回の3分類一覧）。
★ **第3回の予告の「冥王星やケレスも断面で」は、この回で実行しなかった**と明記した。
B：第3回の表の一般的な行を引き継ぎ、この回の設計判断7つを弱で追加。
C：第5回の予告。

## 93.7 通して読んだ印象（実測）
本文の文字数（innerText、閉じている部分を除く）：第3回 約28,200字 → 第4回 約43,200字（約1.5倍）。
長いのは リュウグウとベンヌ 約4,000・彗星 約3,300・オールトの雲 約3,200。
重複：「共鳴は閉じ込めも追い出しもする」が6か所、「押し出されてきたもの」が7か所前後、
3分類の説明が6か所。ボイジャー1号の数値が図・図の説明・本文の3か所。——報告のみ、未着手。

## 93.8 整理（指示による）
- **3分類の説明は §13 に1か所だけ**。「3つの分け方」の箱を置き、「証拠がない≠まだ分からない」
  （リュウグウとベンヌから移した段落）もそこへ。各カードは「〜側です」の適用だけに
  （ベスタ・トロヤ群・冥王星・オールトの雲）
- ボイジャー1号の数値は、オールトの雲の図の説明から削った（図と本文に残る）
- §10 のケレスは1行に
- 「押し出されてきたもの」は §12 にまとめ、彗星・付録A・予告は §12 を指す1行に。
  配置図の説明とリュウグウとベンヌは共鳴と直結するので残した
- ★ **「共鳴は閉じ込めも追い出しもする」の6か所は残す。**到達目標③の軸で、カードごとに
  別の顔（追い出す／閉じ込める／溜まる／追い出された先）を見せている。横断への参照に
  置き換えると、カードだけ読む人に軸が見えなくなる
- リュウグウとベンヌ・彗星・オールトの雲の長さはそのまま（この回でいちばん中身のある3枚）
- 第3回の予告との食い違いは付録Aの記録で足りる
...
# 94 共鳴の仕組みの説明と、メモリーバースへの MathJax

## 94.1 「共鳴とは」を小惑星帯のカードに足した
結果（空く・閉じ込める・溜まる）は何度も出ていたが、**なぜ軌道が歪むのか**の説明が無かった。
カークウッドの空隙の行の直前に「共鳴とは」の行を置いた。
- 周期が整数比 → 何周しても同じ場所・同じ向きで引き合う（2:1 の例：出会う点が毎回同じ）
- 1回の引力はごく小さい（木星は太陽の約1/1000）が、同じ向きの力が積み重なる
- 整数比でなければ出会う場所がずれ、打ち消し合う
- 積み重なった結果が弾き出すか固定するかは、配置で決まる
- ★ 例えはブランコ。**「当てはまるのは積み重なるところまで」と明記**——ブランコには
  「閉じ込める」働きが無いので、例えを広げると不正確になる
カークウッドの空隙・トロヤ群・カイパーベルトの行に、それぞれ「積み重なりがどちら向きに
働いたか」の1文を入れ、§04 の「共鳴とは」を指した。§13 の共鳴の表の前にも仕組みの1文。
「なぜ空くのか」の行の第3回との対比はそのまま。

## 94.2 ★ MathJax をメモリーバース全体に
第1〜4回の `<head>` に、物理数学と**一字一句同じ**設定ブロックとローダー（mathjax@3 tex-svg、
jsdelivr → unpkg の createElement フォールバック、設定はローダーより前）を入れた。読み込んだ版は 3.2.2。
- 第1〜3回は式が0個。`main` の innerHTML のハッシュ・本文の文字数・文書の高さを、
  入れる前と後で突き合わせて**完全一致**（組版された式 0）
- 初期表示：同条件で MathJax なし／ありを交互に各8回、中央値。FCP は第1回 374→442ms、
  第4回 568→498ms で、ばらつきの幅（最小〜最大が数百ms）の中。load は 40〜60ms 遅れる傾向。
  tex-svg.js は転送 603KB（展開 2MB）、キャッシュ無しで取得 約0.4〜0.5秒。**async なので
  最初の描画は待たない**。式のあるページだけに限る必要は無いと判断
- `mjx-container` の溢れ対策（§84）は lesson-theme.css にあり、メモリーバースも読んでいるので
  そのまま効く。390／360px で横スクロール 0
- メモリーバースの目次ページ（memoryverse.html）には入れていない。物理数学の目次ページ
  （phys-math.html）も読み込んでいないのに合わせた

## 94.3 式の書き直し
4回ぶん探して、素の HTML の式は第4回だけだった。TeX にしたもの（計10個）：
ケプラーの第3法則（小惑星帯の本文——変数3つ・別行立て1・計算例1、カイパーベルトの図の説明、
§13 の箱）、共通重心の計算2つ（冥王星とカロンの図の説明）。エラー 0、生の TeX の残り 0。
★ **式のすぐ後ろの「、」「。」が行頭に回った。**インライン数式は §84 で inline-block に
しているので、式と句読点のあいだで改行できてしまう。該当の2か所だけ `<span class="nb">`
（white-space:nowrap）で式と句読点をまとめた。
残したもの：「10万 AU ÷ 27万 AU ＝ 約37%」（文中の割合）と、6.8×10²⁵ km³・9.4×10²⁰ kg
（文中の数値の表記）。式というより数値なので、素の HTML のまま。
...
# 95 第5回 太陽と全体の統合（太陽のカードと太陽風の節）

## 95.1 ファイルと導線
`html/memoryverse_1-5.html` を新規作成。第4回の head と CSS を引き継ぎ（リュウグウとベンヌ専用の
`.pair-*` だけ外し、太陽の層の一覧の色 `.cut-list .l-suncore` ほかを足した）、
MathJax・`js/memoryverse.js`・レール・サイトナビも同じ。CRUMBS に `memoryverse_1-5`、
`memoryverse.html` にカード（副題「真ん中の星から、4回ぶんをつなぎ直す」）。
§02 の最初に「新しく覚えるのは太陽の中身だけ、あとは4回ぶんをつなぎ直す回」と書いた。

## 95.2 太陽のカード（［断面］＋［球］、第3回の巨大惑星と同じ作り）
断面は6層。中心核（〜0.25）・放射層（0.25〜0.7）・対流層（0.7〜1）は縮尺どおり。
光球と彩層は半径の0.3%未満なので**厚みを誇張**と図に明記。コロナは円の外の光（模式）。
★ **指示の「光球＝温度が最も低くなる層」は正確でない**。温度の最小（約4,100〜4,400K）は
光球の上端、彩層との境目（光球の底から約500km）。「上端で最も低くなる」と書いた。
コロナの温度は NASA の現行の書き方（100万℃超、200万℃ほど）に合わせた（「1〜3百万K」は出典なし）。

光が出るまで：**約4万年（NASA SVS）〜約17万年（NASA Sun Facts・ESA・Mitalas & Sills 1992）
〜約100万年（NASA MSFC）**。同じ NASA の中で25倍の幅。理由（1回に進む距離の見積もりと出口の
取り方）を書き、「幅ごと覚える」とした。
コロナ加熱問題は**「まだ確かめられていない」**（波とナノフレアの2説、パーカー、ソーラー・
オービターのキャンプファイア。決着なし）。**「まだ誰も見ていない」は置かない**——内部は日震学と
ニュートリノで調べられているので、手がかりが無いわけではない。
黒点：約3,500〜4,500K、磁場が対流を妨げる。11年、極の反転、22年。第25周期の極大は
SILSO の集計で2024年10月（NASA 自身の極大月の宣言は見つからず、SILSO を典拠にした）。

## 95.3 ★ 太陽の球の画像は本教材で作った
正距円筒の白色光の全球図は公開されていない（HMI の連続光シノプティック図は文書にあるが
JSOC で配布されていない。AIA 193Å の Carrington 図は筋の欠損がある。Solar System Scope と
NASA 3D Resources の太陽は絵）。SDO/HMI のクイックルック連続光画像222枚（2024-06-02〜30、
3時間ごと、Carrington 自転 2285）から、経度ごとに中央子午線にいちばん近い画像を取り
双一次補間で継いだ 2048×1024（左端＝Carrington 経度0°、中央180° → `data-lonc="180"`）。
- 極のまわり 6.3% は写っていないので灰色（埋めない）
- ★ **一瞬の姿ではない**（場所ごとに日付が違い、継ぎ目で約4週間ずれる）、**色は SDO の着色**、
  を球の説明に明記
- ピンは置かない（黒点は動いて消える＝木星の大赤斑と同じ事情）。見せるのは
  「黒点が赤道の両側の帯に並び、極には無い」こと
- クレジット：Courtesy of NASA/SDO and the AIA, EVE, and HMI science teams

## 95.4 太陽風の節（§04）
図：太陽の表面（0.00465 AU）から 1,000 AU まで、第4回と同じ1目盛90.1の対数目盛。
パーカーの最接近（表面から約610万km）、8惑星、末端衝撃波（V1 94／V2 84 AU）、
ヘリオポーズ（V1 2012-08-25・約122 AU／V2 2018-11-05・約119 AU）、ボイジャーの現在地
（172／144 AU、JPL Horizons 2026-09-25）、オールトの雲（推定）。
4回ぶんへの接続の表。★ **第2回に無かった話は「この回で足す」と書き分けた**：
- 金星：第2回は「水が失われた」までで、太陽風の話は無い。しかも「大気が剥ぎ取られている」
  は不正確で、剥がれているのは**水（水素と酸素が 2:1）**（ESA ビーナス・エクスプレス）
- 地球のオーロラ：第2回には「磁場が太陽風を逸らす」までで、オーロラは無い
- 水星：第2回は「太陽風と微小隕石」。元素によって主な仕組みが違う（Mg・Ca は微小隕石）と補った

## 95.5 第2回の記述で古くなっていたもの（→ 直した）
- MAVEN は2025年12月6日に通信途絶、2026年6月3日に運用終了。第2回の「MAVEN はこの流出を
  実際に測っています」（現在形）は古い
- 水星の外気圏「太陽風と微小隕石が叩き出した」は、ナトリウムについては不十分（主に紫外線）

**直した（指示による）**：第2回の6か所（水星のカード・大気の比較表・演習の解答、火星のカード・比較表・
演習の解答）。MAVEN は「測った（2014〜2025年）」という成果と「いまは動いていない（2025-12-06 通信途絶、
2026-06-03 運用終了発表）」という現況を分けた。水星は「ナトリウムは主に太陽の紫外線（Cassidy ほか 2015）、
Mg・Ca は主に微小隕石（NASA 2017）」。第5回の表も同じ文言に揃えた（第5回にはナトリウムの記述が
無かったので足した）。
太陽の球の説明に「継ぎ目のそばでは同じ黒点が2回写る／片側にしか写らないことがある」の1行を足した。
...
# 96 第5回 統合の節（配置図・押し出されてきたもの・巡回・3分類）

## 96.1 統合の配置図（§05）
太陽の表面（0.00465 AU）からプロキシマ（27万 AU）まで約8桁を、第4回・§04 と同じ 1目盛90.1 で。
★ 640 に収まらないので**この図だけ viewBox 800・`min-width:800px`**（狭い画面では横送り、縮めない）。
38枚すべての対象を置いた。色＝そのカードが出てきた回。衛星は母惑星の下に**段違い**
（外側の惑星ほど上の段、引き出し線は自分の段で止める）。★ 木星・土星の衛星名は1行だと
ヘリオポーズの破線を貫いたので2行に。第4回の天体の行は18下げた。

## 96.2 押し出されてきたもの（§06）
表（何が／押し出した力／どこから→どこへ／いつ／分類）と、1本の流れ（できてまもなく→
巨大惑星の軌道が動いた時期→いま）。★ **「いまも続いている」ものは分かっている側、
「できたころ」のものは確かめられていない側にほぼ分かれる**、を箱にした。
典拠：NASA Asteroid Facts（木星が小惑星帯の惑星づくりを止めた）、Tsiganis ほか 2005（ニース・モデル）、
Malhotra 1993（海王星の移動で冥王星が 3:2 に）。移動の時期は議論中と明記。

## 96.3 ★ 全カードの巡回（§07）——取り違えやすい7組の置き方
**並べ替えない**。巡る順は統合の配置図（太陽からの距離）で固定。組のために並べ替えると
「配置図のどこか」から思い出す足場が崩れるため。その結果：
- 隣に来る4組（ガニメデとカリスト、ケレスとベスタ、天王星と海王星、カイパーベルトと散乱円盤）
  ——境目で立ち止まって見分けどころを1つ言う（表の下線 `.pairmark`）
- 離れる3組（エウロパとエンケラドゥス、タイタンとトリトン、イオとエンケラドゥス）
  ——場所そのものが見分けどころ。1周し終えてから3組だけ並べて見分けを言う
見分けどころの表は、各回のカードの記述だけから作った（新しい数値は足していない）。
天王星と海王星は「色では見分けない」（Irwin ほか 2024）。

## 96.4 3分類の全体一覧（§08）
29対象（38枚）×3列。オールトの雲の行だけ背景色（`.known-all tr.both`）。「—」は
「カードで扱っていない」の意味で「分かっていないことが無い」ではない、と注記。
★ 「まだ誰も見ていないのは探査機の通り方のせい」を最初「ほとんど」と書いたが、極の欠け
（ガニメデ・カリスト・ケレス・ベスタ）は周回機でも光の当たり方で撮れていないもの。
半球ごとの欠け（1度の通過）と極の欠けを分けて書き直した。
...
# 97 第5回 残りの節（到達目標・訓練・演習・付録）

## 97.1 到達目標と訓練メニュー
到達目標：①統合の配置図に38枚 ②太陽の断面と太陽風の6か所 ③取り違えやすい7組の見分け。
訓練：新規カードが太陽の1枚だけなので配分を変えた。**Aの最初の3分は毎日「統合の配置図」**、
残り9分で巡回を1区間。★ **1周＝5日**（§07 の5区間を1日1区間）。区間のカード数は
太陽と内側12（2枚ずつの天体が5つ）・小惑星帯3・木星のまわり8・土星から海王星10・外側5＝38。
★ 最初「太陽と内側9枚・外側6枚」と数え違えた（合計38に合わない）。直した。
3週間で約3周（順・逆・2区間ずつの速い周）。同じ天体の2枚は巡回では1回にまとめる。

## 97.2 演習（6本）
演習1は29対象＋太陽圏の境界で30点満点、目安27点。衛星は正しい母惑星の下で1点。
演習2は「光球が最も低温の層」を半分正解とする注記（温度の最小は光球の上端）。

## 97.3 付録
A：第1〜5回の実際の中身と「できた型」、第6〜12回（地球の回）の案、型の使い方の対応表、
接続点のティコ。★ 地球の回の3分類を「確かな数字／推計・議論のあるもの／時点で変わるもの」に読み替える案を
出したが、**採らない**と決まった（§98.1）。
★ 最初「小天体の断面は扱わないことにします」と書いたが、まだ決まっていない判断なので
「扱うかどうかはまだ決めていない」に直した。
B：5回ぶんの手順を1表（強3・中2・弱16）。この回の新しい3つ（統合の配置図を背骨に／
取り違えやすい組を並べ替えずに扱う／3分類を全体に適用）と、太陽の全球図を実写から作る判断を弱で。
並べ替えない判断は交互練習の研究（Birnbaum ほか 2013）と部分的にしか整合しない、と明記。
C：第6回の予告（ティコから地球へ）。

## 97.4 ★ 並行作業との分離
物理数学の作業が同じリポジトリで並行していた（r80・r81 をその側がコミット）。
その側は第1〜4回と目次ページの `?v=` も r81 に上げていたが、**行単位で分けてあり、
こちらの未コミットの変更（第2回の MAVEN・水星など）は取り込まれていなかった**。
新しい第5回のページは r79 のままだったので r81 に合わせてから、layout.js の変更ぶんとして
bump.sh で r82 へ。README の「Current」は r79 のまま止まっていたので r82 に直した。
★ 検査の途中で 8765 番のサーバーが2度落ちた。落ちている間の検査は**エラーページを相手に
「問題なし」と出ていた**。検査の前に、ページの節の数とタイトルを確かめるようにした。
...
## 98.1 ★ 3分類はそのまま。変動する数値は出典と日付（別の軸）
3分類は「何が分かっているか」ではなく**「なぜ分からないか」**を分ける仕組み
（まだ確かめられていない＝手がかりはあるが決め手がない／まだ誰も見ていない＝手がかりがそもそも無い）。
地球でも同じ構造が現れる：民族の起源に説が複数あって決着しない＝確かめられていない。
統計が政治的な理由で公表されない、外部の調査が入れない＝誰も見ていない。
**「時点で変わるもの」は分類ではなく別の軸**——人口や企業の順位が変わるのは、分かっていない
からではなく対象が動いているから。既存の規則「変わりうる数値は出典と日付を添える」で扱う
（衛星の数、探査機の稼働状況もこの規則で処理してきた）。付録Aの対応表を2行に分け、箱で明記した。

## 98.2 重複の整理
「38枚」を16か所→4か所（到達目標①・統合の配置図の説明・巡回の冒頭・訓練の枚数表）。
7組の見分けどころは演習4の解答に要点を残す（解答を読むときに §07 へ戻らせると流れが切れる）。

## 98.3 ★ チェック項目：前の回を直したら、第5回の2か所も直す
第5回の次の2つは、第1〜4回のカードの値と記述から作っていて、**前の回の修正と連動する**。
第6回以降で前の回を直すときは、必ずここも確かめること。
- **統合の配置図（§05、`gen/whole.py` で生成した SVG）**——各対象の位置（軌道長半径、近日点〜遠日点）、
  どの回のカードか（色）、衛星の並び。カードの追加・削除、軌道の値の更新で崩れる
- **3分類の全体一覧（§08）**——各対象の「分かっていること／まだ確かめられていない／まだ誰も見ていない」。
  カードの「分かっていないこと」の記述を直したら、ここの行も直す
- あわせて、**§04 の太陽風の表**（第2回・第4回への接続）と**§07 の見分けどころ**も、
  各回のカードの文言を引いている
★ 実例：§95.5 で第2回の MAVEN を「測った／いまは動いていない」に直したとき、
§04 の表と §08 の火星の行の両方を直す必要があった。
- ✓ **解決：第5回の付録A・Cと、1章の呼び名**（§99 で発生、次のコミットで直した）。
  - 第5回 付録A：地球の回の7行の表（第6〜12回の案）を**削除**し、確定した 2-1 の中身だけを書いて、2章の残りと3章は
    **2-1 の付録Aへ送った**。案を第5回にも持たせると二重管理になり、§98 の連動が増えるため
  - 第5回 付録A「太陽系の回で作った型は、地球でこう使う」の表は残し、2-1 に合わせた：地図と球（正距円筒・1/cos φ で直す）、
    3分類（国境の係争は入れない）、横断する軸（2-1 の cos φ）、統合（「第12回」→ 形はまだ決めていない）
  - 第5回 付録C：「2-1 の予告」。山脈と川を外し、持ち越す型の「分かっていること・推計・時点で変わるもの」を
    「分かっていることと分かっていないこと」に（§98.1）。1章から2章の記事名へのリンクは張らない（前後リンクが持つ）
  - 1章の本文の「第6回（以降）」を「2章（以降）」に読み替え（1-1・1-2・1-4・1-5、HTML のコメント1か所を含む）。
    1-1 に残っていた v3 初期案の予告（第6回以降＝時間軸・銀河層・作業台）は、いまの構成に無いので**触れずに削除**し、
    「2章以降の構成は 2-1 の付録A」に置き換えた。1-2 の「同じ地図の上に」は「同じ枠（正距円筒の展開図と30度の升目）の上に」
    ——第2回の Blue Marble と 2-1 の Natural Earth は別の画像だが、枠が同じだから展開図で覚えたものがそのまま乗る。
    `grep -rn "第6回\|第7回" html/` は0件
- ★ **3章に入る前に決める**：3章を何回に分けるか（M49 の23地域をそのまま使うか、小さい地域をまとめるか）。
  **統合の回の形も一緒に決める**——回の分け方で巡回路の長さが決まるため。2-1 の付録Aには「まだ決めていない」と書いてある
  ✓ **カードの作り方は固まった**（国のページを生成する仕組み、層1・層3は Natural Earth・層2は OSM、0の見せ方、ラベルの規則。§100）。
  ★ **3-1 に遡ってプレートの軸を足す**（2-2 を書いた。3-1 は cos φ だけで書いたので、アルプス＝アフリカとユーラシアの衝突・
  アルプス造山帯の話が入っていない。§102.6）。2-3（気候と海流）を書いたら、3-1 に北大西洋海流も足す
  回の分け方と統合の形は、まだ決めていない
- ★ **地球と人類との重なり**（§103.3）：地球と人類の第1章 §05 を公開したら、2-2 のプレート境界の3つの動きの説明を縮めて参照にする。
  2-3 は、第1章 §06〜§08（気候・海流・山脈）と分担して書く（仕組みは第1章、場所はメモリーバース）
- ★ **2-3 を書いた（§104）**。2-3 の5か所の「地球と人類 第I部 第1章（準備中）」のリンクは、第1章を公開したら行き先を直す。
  ★ **3-1 に遡って足す**：2-2 のプレートの軸（アルプス）と 2-3 の気候の軸（**北西ヨーロッパ +7.3℃**＝中央値・標高1000m未満で比べた値。平均で比べると +7.1℃）、沖の北大西洋海流（0.125 m/s、名前は 0.10 の基準で付けた）を**まとめて1回で**
- ✓ **解決：1章の5記事の `<style>` の残骸**。`  .lesson-nav .next,.lesson-nav .mid{text-align:left}}`（メディアクエリの開き行を
  失った1行）が **1-1〜1-5 の5記事すべて**にあった。★ **害は text-align ではなかった**——`lesson-theme.css` が後から同じ
  詳細度で `.next{text-align:right}` / `.mid{text-align:center}` を持つので、この1行は最初から上書きされて効いていない。
  **余った `}` が直後の `.progress-wrap{…}` を巻き込んで無効にし、5記事ともレールの進捗バーの台が高さ0で消えていた**
  （バー自体も台の高さ100%なので見えない）。復元ではなく削除（入るはずの `@media` の条件が分からず、同じ役割は
  `lesson-theme.css` が持つ）。2-1 には最初から写していない。
  ★ 確かめ方：削除の前後で、5記事×3幅の全要素の計算済みスタイルを比べた。宣言が変わったのは `.progress-wrap`
  （0→2px・背景・角丸）と、その子の `.progress-bar`（高さ0→2px）だけ。ほかは幅が最大0.016px動いただけ（描画の丸め）。
  削除後、5記事とも台2px、半分で50%・末尾で100%。前後リンクの揃えは変化なし

## 98.4 State
r82 のまま（html と DESIGN.md のみの変更。css/js は変えていないので ?v= は上げない）。

---

# 99 2-1 地球の骨格（2章 地球の最初の記事）

## 99.1 ★ 2章に入る前に決めた4点と、番号の付け方
| 決めたこと | 判断 | 理由 |
|---|---|---|
| **図法** | 全球は**正距円筒図法** | 緯線・経線が等間隔の直線で、30度の升目がそのまま読める。歪みは東西方向だけで 1/cos φ の1本の式で頭の中で直せる（メルカトルは面積が 1/cos²φ、緯線の間隔も不揃い）。同じ画像をそのまま球のテクスチャにできる（§77.1 の条件） |
| **カードの単位** | **地域が1枚、国はその中の引き出し** | 惑星の下に衛星を置いた入れ子（§85）と同じ。約200か国を1枚ずつにすると、配置図のような背骨が作れない。2-1 は地域に入る前の枠なので、カードは3枚（大陸と海／基準線／錨の11か国） |
| **順番** | 各地域・各国の**中心の経度で、東経0度から東回り** | 太陽系を軌道の順（測れる数）で並べたのと同じ。東回りは地球の自転の向き。中心は球面上の重心（§99.3） |
| **国境** | **実効支配で線を引き、主張・承認は別の欄**。3分類には入れない | 3分類は「なぜ分からないか」の仕組み（§98.1）。領土の争いは3つの欄がどれも「分かっていること」で、決着しないのは政治と法の問題。入れると発見を待つ正解があるように見える |
| **記事の番号** | 1章は通し番号（第1〜5回）のまま、**2章から章ごとの番号**（2-1、2-2…）。ファイルは `memoryverse_2-1.html` | 章の見出しで区切られるので「1章 太陽系 > 第1回」と「2章 地球 > 2-1」が並んでよい。1章のラベル・タイトル・ファイル名は変えない。§36.1 の `<科目>_<章>-<節>` の命名にも合う |

★ 最初は `memoryverse_1-6.html`・「第6回」で作り、コミット前に 2-1 へ改めた。CRUMBS のキー・`data-node`・`<title>`・
レールの `.brand`・`<h1>`・一覧ページのカード・本文中の「第6回／第7回」（→「この回」「2-2」）を合わせて直した。

## 99.2 地図画像（`image/memoryverse/earth-{countries,anchors,lines}.png`）
Natural Earth **タグ v5.1.2**（master は 5.2.0-pre なので使わない）から Python（shapely・pyproj・Pillow）で生成。
GeoJSON と生成スクリプトはリポジトリに入れていない（作り直すときは下の仕様から）。
- 2048×1024、正距円筒、**左端＝西経180度**、余白なし。`x=(lon+180)/360·W`、`y=(90−lat)/180·H` のみ
- 4096×2048 で描いて LANCZOS で縮小 → 96色・ディザなしで量子化。各約200KB
- 描画順：海 → 陸（`ne_50m_admin_0_countries_lakes`）→ 係争地の斜線（`ne_10m_admin_0_disputed_areas` の
  Disputed/Breakaway/Indeterminate。右上がりの斜線をマスクと掛け合わせ、輪郭も引く。**塗りつぶさない**）→ 国境
- `earth-lines.png` の西サハラ：Natural Earth の「Western Sahara」は壁の東側（ポリサリオ戦線の支配域）だけで、
  西側はモロッコの形に含まれる。北回帰線は両方を通るので、**係争地データの W. Sahara の範囲（西側）も茶色に塗った**
  （モロッコ本体は塗らない）。南極は塗らない
- **較正**：東京・ロンドン・ケープタウン・シドニー・ニューヨークに印を打って拡大し、目視で海岸線上に乗ることを確認
- 地図カードは第2回の地球と同じ `data-grid="equi" data-lon="we"`、`data-lonc` なし。画像に `?v=` を付けないのも第2回と同じ
  （`data-globe` 側だけ付く）。基準線は 1440px で 30度ごとの全線が画像の割合と一致（実測）

## 99.3 ★ 数値は計算して、渡された期待値と突き合わせた——期待値のほうが違っていた
面積は `Geod(ellps='WGS84').geometry_area_perimeter`。(a) 面積上位・南極を除く陸 13447万km²・11か国56%／9か国52%、
(b) 5本の線（線分との交差）、(c) 東経90度・西経90度、(d) 見かけの面積の倍率（正距円筒・EPSG:3395）、
(g) 係争地の点の判定——**一致**。
★★ (e) 錨の中心と (f) 地域の中心：渡された期待値は「**ポリゴンの部分ごとの平面上の重心**を、楕円体上の面積で
重みづけして単位ベクトルで平均」した値で、**球面上の重心ではなかった**（この方法なら小数第1位まで一致したので特定できた）。
記事は「球の上の重心」と書くので、値を**真の球面重心**に差し替えた。
- 計算：国の形の部分ごとに細かい格子（0.1度以下、小さな島は部分の大きさの1/60）を当て、中に入る点の単位ベクトルを
  cos(緯度) で重みづけして平均し、部分どうしは楕円体上の面積で重みづけして足す。格子を 0.04度にしても小数第1位は同じ
- ロシアは 北62.0°東99.4° → **北66.0°東95.7°**、カナダは 北59.9°西99.6° → **北60.4°西96.5°**。ほかは 0.6度以内の差
- **(e) の東回りの順は変わらない**（2.7／23.6／67.3／79.6／95.7／103.4／134.3／256.3／263.5／295.2／306.8）
- **(f) は1組入れ替わった**：中央アメリカ（西経98.6度）が北アメリカ（西経98.0度）より先になった（旧：北アメリカ
  西経99.6度が先）。差が1度もないので、付録Aに「方法やデータで入れ替わりうる」と書いた
- ★ **輪の切れ目は東経0度**（字どおりの「東経0度から東回り」）。最初は西ヨーロッパ（東経0.9度）、最後は西アフリカ
  （西経1.1度＝東経358.9度）。渡された構成では西アフリカが出発だったが、それだと規則が字どおりでなくなる。
  「0/0 にいちばん近いものから」にすると、錨ではコンゴ民主共和国がアルジェリアより近く、リストごとに規則が変わるので採らない。
  切れ目の説明を付録Aに箱で置いた（展開図の左右の端が同じ180度の線であるのと同じ、輪にも切る場所が要る）。
  §05 の東経0度を北から南へたどると 北ヨーロッパ（イギリス）→ 西ヨーロッパ（フランス）→ 南ヨーロッパ（スペイン）→
  北アフリカ（アルジェリア）→ 西アフリカ で、輪の最初の4地域と最後の1地域が切れ目の線の上に並ぶ。
  **地域の回は3章**で、最初は西ヨーロッパ（付録Cの予告もそう直した）
- ★ **2章と3章の区別**（付録Aの表）：2章＝**全地域に1本で通る仕組み**（cos φ の日射、プレートの境界、海流）。
  3章＝地域ごとに**国の形と場所＋2章の仕組みのその地域での現れ方（気候・地形）＋その地域にしかないもの**（産物・民族・企業・観光地）。
  第5回の予告の「気候」の回は 2-3 として残し、全球の気候帯を置く回にした（国ごとの気候は3章）
- ロシアを含めた東ヨーロッパの中心は東経87度（旧92度）。ウクライナ全土をロシアに足した上限でもロシアは東経92.8度
- ★★ **（後日）中心の計算は「本土だけ」に変えた（§100.10）**。この節の値は領土の全体で計算したときのもの
- ★ **地図のピンは動かさなかった**（ラベルの向きの調整 §99.5 を崩さないため）。表の中心は計算値、ピンは読みやすさの
  ための位置、と脚注に書いた（アメリカ合衆国の本土のピンと同じ扱い）
- ★ **数は表の1か所だけ**：錨のピンのラベルから緯度経度を外し、短い手がかりにした（「ロシア／180度の経線が通る」など）。
  同じ「中心」に表とラベルの2つの数が並ぶと、どちらを覚えるかで迷う。大陸と海のカードのラベルの度数
  （「本体は北緯30〜60度」など）は中心の値ではなく升目の説明なので残した
- 北回帰線の交差で「Morocco」が出るのは、モロッコの形に含まれる西サハラ西側（上の §99.2）
- ★ チャドの北端（23.4447度）と2026年の回帰線（23.4358度）の差は約1km で、1:1000万のデータの位置の精度（数km）より
  小さい。**そこから年代を逆算するのは、データが持っていない精度を使うことになる**ので書かない。本文は「入るかどうかは
  年だけでなくデータでも変わる」と、データの限界として書いた（最初の版は年代を逆算していた）

## 99.4 記事（`html/memoryverse_2-1.html`）
第5回の `<style>` から、使う部品（レール・カード・地図・球・演習・表・箱・`.known`）だけを写した。値は同じ。
外したもの：衛星の目次の階層（`.toc .sub`・`.parent`・`.sec-head.is-sub`）、`.map-note`、`.vsw-note`、`.geo-far`、
環・近赤外・断面・太陽の層の色・`.pairmark`・`.known-all`。第5回の残骸の1行（§98.3）は写していない。
- §07 のグラフは SVG（viewBox 640）を `.fig-scroll` に入れた（390px で横送り、§90.1）
- 球のボタン：`js/memoryverse.js` の `syncOpen()` を `data-open-label` で差し替えられるようにした（無ければ「地形」）。
  1章は属性を持たないので「地形を開く」のまま（実測で確認）

## 99.5 ★ ラベルの向き（`data-side`）は渡された値から変えた
渡された値のままだと、1440px でオーストラリア大陸（右）が自分のピンを覆って南極海と重なり、
基準線のカードは南回帰線と東経0度が重なった。1024／768px ではさらに増える（ラベルは固定の大きさ、地図だけ縮む）。
1440／1024／768px の3幅で「ラベル同士の重なり（4px の余白込み）＋ラベルが他のピンを覆う」を数え、
**0 になる組み合わせを、渡された値から少ない変更で**探した。
- 大陸と海：1 右／2 下／3 右／4 下／5 上／6 左／9 下／11 下
- 基準線：2 上／3 下／4 右／5 上
- 錨：1 アルジェリア **左**（渡された値は下。下だとコンゴ民主共和国のピンを覆う）／8 アメリカ合衆国 **下**（右から）／7 オーストラリア **既定**。
  あわせてアメリカ合衆国のラベルを短くした（のちに錨のラベルはすべて緯度経度を外した。§99.3）。外したあとも3幅で重なり0
- 390px の番号の丸は、錨の 3・4・6 と 10・11 が数pxずつ接する（`mvSpread` の押し離しの範囲内。番号はすべて読める）

## 99.6 付帯の変更
- CRUMBS に `memoryverse:ch2`（2章 地球）と `memoryverse_2-1`（label `2-1`）。目次ページに「2章 地球」の節とカード
  （説明は1章と同じく**記事の副題**「ティコから降りて、置き場所の枠を作る」。渡された説明文は `<meta name="description">` に入れた）。
  目次ページのリード文を「1章（第1〜5回）で太陽系を、2章（2-1から）で地球を扱う」に直した
- キャッシュバスター：渡された手順は r83→r84 だったが、すでに r84（`1d75742`）だったので **r84→r85**。
  `tools/bump.sh` で html の `?v=` を一括（**画像の `data-globe` の query も含む**。過去のバンプ `1d75742` でも第2回の
  `data-globe` 10か所が一緒に上がっている）。README の Current は bump.sh の置換が効かず r82 で止まっていたので手で r85 に

## 99.7 State
`395406a`（r85、2-1 本体）、`ece0757`（1章の読み替え・進捗バーの復旧）。§99.8 の比較図は次のコミット（r85 のまま）。

## 99.8 ★ 見比べる図は横送りにしない（§07 グリーンランドとコンゴ民主共和国）——§90.1 の例外
§07 の倍率の箱に、同じ2か国を同じ縮尺で2通りに描いた図を足した（2行×2列。左の列＝展開図のまま・赤道の縮尺、
右の列＝それぞれの球面重心を正面にした正射図法。左右とも赤道での1度を同じ長さに）。
- 右の列は倍率ではなく実面積（約215万km²／約233万km²）。右は実物なので、倍率だとどちらも「1倍」になって情報が無い
- 正射図法は面積を保たない。グリーンランドは中心から広がるぶん**面積で約2%小さく**描かれ、右だけ見ると
  コンゴ民主共和国がその約1.11倍に見える（実際は1.08倍）。figcaption に明記した
- 形は Natural Earth v5.1.2 から0.1度（約0.5px）で簡略化し、小さな島を除いた。簡略化後も面積の比は保たれる
  （グリーンランド 3.85倍、コンゴ民主共和国 1.01倍）。SVG は約27KB。グリーンランドの中心（北73.1° 西41.8°）は §99.3 と同じ方法

★★ **この図だけ §90.1（図は縮めずに横へ送る）の例外にした。理由はこの図に固有のもの。**
- §90.1 は、**細かい注記が縮んで読めなくなることを避ける**ための規則だった。この図の文字は国名と倍率だけで、
  細かい注記が無い
- **この図は左右を見比べることが中身そのもの**。片方ずつ見せると図の意味が消える。実際、横送りのままだと
  390px で最初に見えるのは左半分だけで、「グリーンランドは大きい」という、**まさにこの節が正そうとしている誤解のほうだけ**が見えた
- したがって例外の条件は「縮めても文字が読めるから」ではなく、**図の中身が同時に見えることに依存しているかどうか**。
  依存していない図（cos φ のグラフ、第1〜5回の配置図など）は §90.1 のまま横に送る。同時に見えることに依存する図を
  次に作るときも、この条件で判断する（4つを縦に並べる案は、左右の比較という構造が崩れるので採らなかった）

実装：
- `<figure class="fig fig-cmp">` の直下に SVG を置く（`.fig-scroll` で包まない。`min-width:640px` が効かず、本文幅に縮む）
- SVG の中に `cmp-h`（見出し）・`cmp-n`（国名）・`cmp-v`（倍率・面積）・`cmp-r1`/`cmp-r2`（行）・`cmp-scale`（目盛の行）のクラス
- 記事の `<style>` に 560px 以下だけのブロック：文字を大きくする（国名 12→26、倍率 10.5→22、見出し 12.5→20。
  単位は viewBox の座標なので、図と一緒に縮む）、ラベルと2行目を `transform:translateY` で下へずらす（大きくした文字が形に
  重ならないよう、隠した目盛の行の空きを使う）、図の枠の余白を詰める、**目盛の行を隠す**（補足なので、消えても主旨は保たれる）
- 実測：390px で見出し13px・国名16px・倍率14px、360px で11/15/13px（文字の枠の高さ）。文字と形の重なり0、SVG の外へのはみ出し0。
  1440/1024/768px は変更前とピクセル単位で同じ（背景の星を除く）。cos φ のグラフは 390/360px で今までどおり横送り

---

# 100 3章の器：国のページを生成する仕組み（日本・チャドの試作）

## 100.1 ★ 3章の入れ子——地域の記事（手で書く）の下に、国のページ（生成する）
- **地域の記事**（3章で18〜20本、手で書く）：その地域の国の形と位置、2章の仕組みがそこでどう現れるか、訓練メニュー、演習。1章・2章と同じ型
- **国のページ**（最終的に約200枚、生成する）：層1・層2・層3を重ねたカードだけ。**訓練は置かない**（覚える段取りは記事が持つ）。
  訓練は記事、参照は国のページ
- 国のページは `tools/memoryverse-cards/draw.py` が書き出す。**国を1つ足す＝`countries.py` に1項目足してスクリプトを走らせる**。手で書かない
- 置き場所：`html/country/<国>.html`（`html/` に206ファイルが混ざると既存の記事が埋もれるので分けた）、
  SVG は `image/memoryverse/cards/`。今回は日本（`jpn.html`、図2枚）とチャド（`tcd.html`、図1枚）

## 100.2 ★ 国のページは CRUMBS に登録しない。戻る導線を1本置いて代える
- 206行足すとサイト構成ナビの木が使いものにならない。木は「メモリーバース > 3章 各地域 > 3-1」までで止め、国のページは記事から入る枝葉にする
- `<body data-page="study">` のみで **`data-node` を付けない**（layout.js の `mountCrumbs` は node が無ければ何もしない。
  ヘッダー・フッター・星空は data-node に関係なく出る）
- 代わりに h1 の上に「← メモリーバース」を**手で書いた** `.page-up` で置く（一覧ページで layout.js が書き出すものと同じ形。
  `.rail-up` と見た目が揃っている＝§69.1）。**いまは一覧ページへ戻す**。地域の記事ができたら `countries.py` の
  `up_href` / `up_label` でその記事へ向ける
- ★ **`<base href="../">`**：国のページは1階層深いが、layout.js がヘッダー・フッターに「index.html」のような相対リンクを
  書き出すので、そのままでは `html/country/index.html` を指して404になる。layout.js（共通）を直す代わりに、国のページだけ
  base を `html/` に合わせた。副作用は「#アンカー」が `html/#…` を指すことだけで、国のページには #リンクが無い。
  CSS/JS/SVG のパスもほかの記事と同じ `../css/…` の形になる（`../../css/…` ではない）。ファビコンは `../../favicon.ico`
  ★ `<base>` はページ内のすべての相対URLの基準を変えるので、**`#` で始まる内部リンクは同じページではなく親を指す**。
  国のページに目次のような内部リンクを足すときは、`href="country/jpn.html#…"` のようにファイル名から書くこと
  （base が `html/` なので、`html/` から見たパス。`jpn.html#…` だけだと `html/jpn.html` を指す）

## 100.3 ★ 層1・層3は Natural Earth v5.1.2、層2は OpenStreetMap。層2は本数で切る
Natural Earth だけでは層2が足りなかった（日本の川が3本、湖1つ、道路と鉄道は全部無名、信濃川が無い）。層2だけ OSM に替えた。
Geofabrik の国の抽出を osmium で絞る（Overpass は日本の大きさだと不安定）。出典欄にライセンスと**抽出の時刻**を必ず出す（ODbL は表示が条件）。

| 層2 | タグ | 絞り込み |
|---|---|---|
| 川 | `waterway=river` | その国の中の長さで長い順に15本。同じ名前の区間はつないでから測る |
| 湖 | `natural=water` かつ `water=lake` | その国の中の面積で広い順に10 |
| 高速道路／主要幹線 | `highway=motorway`／`trunk` | 全部（`_link` を除く） |
| 高速鉄道／主要鉄道 | `railway=rail` かつ `highspeed=yes`／`usage=main` | 全部（重なるものは高速鉄道へ） |

- ★ **本数で切る理由**：「主要な川」の長さは国によって違う。日本で100km以上と決めるとチャドでは0本になる。本数なら、どの国にも同じだけの骨組みが残る
- 川は **OSM の名前ごと**に数える。1本の川でも区間で名前が変われば別（信濃川15位と千曲川9位、チャドの Chari と「Chari شاري」、Ba Illi と Ba-Illi）。
  つなげる判断を入れると国ごとに基準が揺れるため
- 層1・層3：都市は `POP_MAX` の多い順に20（`scalerank` は世界地図での重要度で、国のカードには合わない——横浜・神戸が落ち、酒田・釧路が入った）。
  地方は Natural Earth の `region` のまま（日本は沖縄が独立して9地方。佐賀・長崎は値が欠けているので九州で補う＝欠けの補いで、区分の変更ではない）
- 国境は 2-1 §08 のとおり実効支配。北方領土・竹島は日本の範囲に入らないので層2も落ちる。カードの注記に1文で書いた
- 数（2026-09-24T20:21:20Z の抽出）：日本は川15・湖10・高速道路52,147区間（名前99.9%）・主要幹線127,841（99.8%）・
  高速鉄道13,076（93.3%）・主要鉄道64,619（98.6%）。チャドは川15・湖10（名前9）・主要幹線513（64.9%）、ほかは0

## 100.4 ★ 0のものは埋めない：ボタンを薄くして「なし」、図には何も足さない
チャドで**高速道路・高速鉄道・主要鉄道・地方の4つが0**だった（OSM の `railway=rail` は1区間だけで `usage` が無い。Natural Earth に地方の区分が無い）。
- ボタンは破線の枠・不透明度 .55・赤い「なし」・`disabled`（押しても何も起きない。層のボタンを押しても点かない）
- ボタンの下に1行：「この国のデータに無いもの：…（0なので線も名前も描いていない）」
- 図には何も足さない。**無いものは無いと見える**ことが目的（ミランダに球を作らなかったのと同じ扱い）

## 100.5 ★ ラベルの規則
- **置く順は動かないものから**：湖→川→高速鉄道→高速道路→主要鉄道→主要幹線、同じ種類の中は長い順。川と湖は地形で何百年も動かないが、
  道路と鉄道は増えたり廃止されたりする。長さだけで順を決めると国道のラベルが川を押しのけた（川15本中5本しか名前が付かなかった）
- 重なりは**層ごと**に判定する（層は1つずつ重ねて使う。全部を同時に出した面は確かめのためのもので、覚えるための面ではない）。
  **図の枠から出るラベルは置かない**（Chrome で実際の文字の箱を測って、はみ出し0・同じ層の中の重なり0を確かめた）
- **1行の上限**：全角14文字（半角は0.5文字）。超えたら末尾を「…」
- **文字体系が混ざった名前**：空白で区切った語の文字体系が変わったところで切って先頭だけ（「Lac Teli بحيرة تيلي」→「Lac Teli」）。
  1つの文字体系だけの名前（アラビア文字だけ・キリル文字だけ）はそのまま。例外は日本の実物を見て決めた2つ：`JR` のような3文字までの
  大文字の略号は切り替えに数えない（「JR 山陰本線」が「JR」になるため）、「JR東北本線」のように1語の中で漢字かなと混ざるものは切らない
- **経路の説明になっている名前**（チャドの「Abéché - Ati - N'Djaména」）は直さない。「どこまでが名前か」の判断を入れると国ごとに揺れる
- **切り詰めで重複したものだけ後ろを残す**：同じ図の中で同じ文字列になったもの（「Route Nationale N'Djaména…」×2）だけ、先頭を「…」にして
  後ろを残す（「…N'Djaména - Sarh/Bongor」「…N'Djaména - Bokoro - Mongo」）。「起点 - 経由 - 終点」の形は前が共通で後ろで分かれるため。
  全部に当てると「…東北自動車道」のように意味が消える。それでも同じなら区別しない。日本は重複が0で、SVG はバイト単位で変わらなかった

## 100.6 ★ SVG は別ファイルにして、カードを開いたときに取りに行く
- 日本の本土の図だけで 357KB。記事に埋め込むと1地域で数MBになり、**カードを直すたびに記事が変わる**
- ページには枠とボタンだけを書く。`js/country-card.js` が、カードが画面に近づいたとき（IntersectionObserver、400px 手前）に
  `fetch` で SVG を取り、枠の中に差し込む。`<img>` では層の切り替え（SVG の中の class を CSS で消す）が効かないため中身を文書に入れる
- 取ってくるまでは「地図を読み込み中…」、失敗したら「地図を読み込めませんでした」（2-1 の `.planet-img .fallback` と同じ考え方）。
  枠は SVG の縦横比（`--w` / `--h`）で先に高さを持つので、届いたときに下の段落が跳ねない
- SVG の参照にもキャッシュバスターを付ける（`?v=r90`）
- 共通化：カードの CSS は `css/country-card.css`、JS は `js/country-card.js`（部品名は `cc-` で始める）。**地域の記事にカードを差し込むときも
  同じ2ファイルを読む**ため、国のページの中ではなく共通ファイルにした
- SVG の中の `<style>` は、文書に差し込むと**文書全体に効く**ので `.cc-map text{…}` に閉じた。clipPath の id も国ごと（`cc-jpn-main`）にした
  （1つの記事に2か国のカードを置いても衝突しない）
- 狭い画面では図を縮めずに横へ送る（§90.1。図の中の 9px の文字が約8px になる .88 倍で止め、あとは送る）。390px で文書の横はみ出し0

## 100.7 ★ 生成スクリプトを `tools/memoryverse-cards/` に置く
- 基準を変えたときに作り直せなければ、再現できる教材にならない。`extract.sh`（取得と osmium の絞り込み）・`prep.py`（層2の選別）・
  `draw.py`（SVG と国のページ）・`countries.py`（国ごとの違いだけ）・`prep_util.py`（線の扱い・ラベルの名前）・`centroid.py`（標準緯線）・
  `page_tpl.html`（国のページの型）・`README.md`（データの取り方・走らせ方・基準・国を足す手順）
- 取ってきたデータと途中のファイル（`.osm.pbf`・Natural Earth の GeoJSON・`*.geojsonseq`・`data/`）は**入れない**（サイズと ODbL）。
  リポジトリ直下と `Academic-Gate_hp/` の両方の `.gitignore` に足した
- 線をまとめたあとの入れ物に注意：`unary_union` した線を矩形で切ると交差点の点が混ざった `GeometryCollection` が返る。
  `lines_of` で再帰的にほどいて線だけを取り出す（試作ではこれで鉄道が消えた）
- 線網の間引き：図の上の0.5pxの格子に寄せてから重なりを消し、つなぎ直す（上下線・複線が1本になる）。2px未満の断片を落とす案は、
  線全体の1〜2割が消えて途切れるので採らなかった。日本の本土の SVG は 718KB → 357KB

## 100.8 付帯の変更
- 一覧ページ（`html/memoryverse.html`）：2章の節の中に、既存の `.eyebrow` で「3章の国のページの試作：日本 ／ チャド」を1行。
  3章の記事がまだ無いので章の見出しは立てない。新しいスタイルは足していない。地域の記事ができたらこの行は消す
- キャッシュバスター：渡された手順は r85→r86 だったが、物理数学の並行作業ですでに **r89**（`ca14a7b`）まで使われていたので
  **r89→r90**（衝突を避けて次の空き番号）。`tools/bump.sh` は `html/` を再帰で見るので `html/country/` も一緒に上がる。
  README の Current は bump.sh の置換が効かず r85 で止まっていたので、手で r90 に

## 100.9 State
r90。1章・2章の記事は `?v=` の書き換えだけで中身は変えていない（読み込み時のエラー・404は0、サイト構成ナビの木は変わらない）。
3章の回の分け方と統合の形は §98 のとおりまだ決めていない。

## 100.10 ★ 中心は「本土だけ」で計算する（国と地域を並べる規則の変更）
**変更前**：国の**領土の全体**で計算した球面上の重心の経度で、東経0度から東回りに並べていた。
**変更後**：国の**本土＝面積（楕円体の上）がいちばん大きい陸のかたまり**だけで計算した重心にした。計算方法（細かい格子の点を
cos(緯度) で重みづけ、部分どうしは楕円体上の面積で重みづけ）は §99.3 と同じ。地域の中心は、属する国の本土だけを集めて計算する。
- ★ **理由**：海外領土を含めると中心が本土から離れ、並べる順が実際の位置と合わなくなる。フランスは仏領ギアナ・レユニオンなどを
  含めると中心が**西経6.7度・北緯43.0度（大西洋の上）**になり、東経0度から東回りだと西ヨーロッパ9か国の**最後**に来る。
  本土だけなら**東経2.5度・北緯46.6度**で、いちばん西＝最初になる。オランダ（カリブ海）・イギリス・アメリカ合衆国（アラスカ・
  ハワイ）・デンマーク（グリーンランド）・ポルトガル（アゾレス）・スペイン（カナリア）でも同じ向きのずれが起きる
- 「本土＝面積がいちばん大きい部分」は面積を比べるだけなので、**国ごとの判断が入らない**
- 本土に絞ると中心が2度以上動く国（Natural Earth v5.1.2）：フランス 7.4度、アメリカ合衆国 6.1度、キリバス 9.8度、
  セントヘレナ 8.6度、ポリネシア（仏領）5.0度、マレーシア 5.0度、フィリピン 4.5度、インドネシア 3.8度、セーシェル 3.8度、
  ミクロネシア連邦 2.9度、ニュージーランド 2.8度、ノルウェー 2.6度、カナダ 2.2度

**影響した範囲と結果**
- **2-1 §06 錨の11か国**：★ **カナダとアメリカ合衆国が入れ替わった**（8番カナダ 北58.6° 西99.0°、9番アメリカ合衆国 北39.9° 西98.8°。
  旧はアメリカ合衆国 北44.8° 西103.7°＝アラスカとハワイで西へ引っぱられて先、カナダ 北60.4° 西96.5°）。**差は経度0.2度**しかないので、
  脚注に「方法やデータで入れ替わりうる」と書いた。ほかの9か国は順が変わらず、値の動きは0.5度以内（ロシア 北65.7° 東95.2° など）。
  地図のピンは**番号だけ**入れ替え（DOM の順もカナダを先に）、位置（`data-x/y`・`data-lat/lon`）と向き（`data-side`）は動かしていない。
  脚注の「アメリカ合衆国の中心がカナダより西に出るのはアラスカとハワイを含むため」は事実と合わなくなったので削り、本土だけで計算する
  ことと理由（フランスの例）を書いた。演習の答え（東回りの順）もカナダ→アメリカ合衆国に直した
- **2-1 付録A 23地域**：**順は変わらなかった**。値が変わったのは 西ヨーロッパ（北47° 東1° → **北49° 東6°**）、ロシア（東96° → 東95°）、
  東南アジア（北8° 東110° → **北13° 東106°**）、メラネシア（東148° → 東146°）、ミクロネシア（北8° 東160° → 北9° 東159°）、
  ポリネシア（西160° → **西165°**）、北アメリカ（北56° 西98° → **北53° 西96°**）。「入れ替わりうる組」の注記は、中央アメリカと北アメリカ
  （差が 0.6度 → 2.5度に広がった）から、**西ヨーロッパと南ヨーロッパ（東経6.2度と6.6度、差0.4度）**に替えた。輪の切れ目の箱の
  「最初は西ヨーロッパ（東経1度）」を東経6度に
- **2-1 の付帯**：§07 の表の手がかり「カナダ：中心が北緯60度」→ 59度（記事の中の「中心」の意味をそろえるため。倍率の列は領土の全体の
  面積の比なので変えない）。§07 のロシアとウクライナの例の数（東経95.7 → 95.2、92.8 → 92.2、インド 79.6 → 79.5）
- **生成スクリプト**：`centroid.py` を本土だけに（`mainland()` = 楕円体上の面積が最大の部分）。**日本の標準緯線は北緯37.43度 → 36.63度**
  （本州。cos 0.7941 → 0.8025）、チャドは1つのかたまりなので15.28度のまま。日本のカードの SVG を作り直した：本土の図の幅 703 → 710px、
  南西諸島 340 → 342px、ラベルは層2 89 → 88・層3 62 → 63（主要鉄道 +1・主要幹線 −2・都道府県名 +1）、枠からのはみ出し0・同じ層の中の重なり0
  （Chrome で実測）。カードの出典欄の「標準緯線」の説明と README に「本土」を足した
- 2-1 の地図のラベル：1440/768/390px で錨の地図の重なり0。390px の基準線の地図の「赤道／南回帰線」の重なりは、変更前（HEAD）にも
  同じようにあった（この変更とは関係ない）
- キャッシュバスター：CSS/JS は変えていないが、日本のカードの SVG を**同じファイル名で**作り直したので上げた（開いたことのある
  ブラウザに古い地図が残らないように）。物理数学が r92（`43c51cb`）まで使っていたので **r92→r93**。README の Current は r90 で止まっていたので手で r93 に


## 100.11 ★ 倍率を上げる国と、地域のカード（3-1 で決めた）
- **地域のカードも国のページも1度44px**（経度は標準緯線の cos で縮めたあと）。地域のカードと国のページが**同じ縮尺で並ぶ**ことに
  意味がある（地域のカードのドイツと国のページのドイツが同じ大きさ）。標準緯線は、地域のカードが地域の中心（3-1 は北緯49度＝
  9か国の本土の重心 48.50度を丸めた値）、国のページが国の本土の中心
- ★ **倍率を上げるのは、1度44pxで図の長いほうの辺が100px 未満になる国だけ**（ルクセンブルク32px・リヒテンシュタイン9px・
  モナコ2px）。倍率は、長いほうの辺が300〜700px に収まる 10・20・50・100・200倍のうちいちばん小さいもの
  （ルクセンブルク10倍・リヒテンシュタイン50倍・モナコ200倍）。ベルギー126px・オランダ145px・スイス157px は小さいが形は読めるので
  上げない（同じ縮尺で並ぶことを優先）。上げた図は、図の右上（黄色）とカードの注記に**「この図だけ○倍（1度＝○px）」**と書き、
  1度・5度の線がほとんど枠に入らないので細い点線の格子（ルクセンブルク0.2度・リヒテンシュタイン0.05度・モナコ0.01度）と
  そのラベルを足す。縮尺の棒は図の3分の1に収まる長さ（100・50・20・10・5・2・1・0.5km から）
- 記事の拡大図（3-1 §04 の層2）は、カードに収めるための別の枠で、リヒテンシュタイン20倍・モナコ100倍（国のページと倍率が違う。
  どちらも図に書いてあるので混ざらない）
- 地域のカードは `tools/memoryverse-cards/region.py` が描く（形・首都・拡大図の3枚の SVG。§06 の見比べる図は記事に埋め込む）。
  部品は国のページと同じ `css/country-card.css`・`js/country-card.js`。層のボタンは記事ごとに意味を決める
  （3-1 は §04 が層1 形・層2 拡大図、§05 が層1 形・層3 首都。拡大図の枠そのものに `L2` を付けて、層2を切ると枠ごと消える）
- `draw.py` の日本向けの決め打ちを設定にした：区分をまとめる単位の呼び名（`region_word`。日本「地方」、フランス「地域圏」、
  ベルギー「地域」）、名前の後ろの語（`region_suffix`）、字の大きさ。小さい図に長い名前が入らないときは置かずに数える
  （ベルギーの「ブリュッセル首都圏地域」）。SVG のパスの道具は `svgutil.py` に分けた（`draw.py` と `region.py` で共通）。
  日本とチャドの出力はこの変更の前後でバイト単位で同じ

---

# 101 3-1 西ヨーロッパ（3章 各地域の最初の記事）

## 101.1 ★ 3章の1本目。記事は位置を置く、国の詳細は国のページ
- `html/memoryverse_3-1.html`、`data-node="memoryverse_3-1"`。CRUMBS に `memoryverse:ch3`（3章 各地域）と `memoryverse_3-1` を足した
  （1章・2章の行は変えていない）。前後リンクは 2-1 ↔ 3-1。国のページ9枚は CRUMBS に入れず、戻る導線を 3-1 に向けた
  （`countries.py` の `up_href`）。日本とチャドは属する地域の記事がまだ無いので一覧ページのまま
- 型は 2-1。`<style>` は 2-1 から、この回が使う部品（レール・見出し・箱・表・図・見比べる図・到達目標・演習）だけを写した。
  地図のピン（`.mod`/`.orbit`/`.stat`）と球（`.globe`）は使わないので持たない。560px 以下の地図用のブロックも持たない
- 一覧ページに「3章 各地域」の節と 3-1 のカードを足し、日本とチャドの試作の1行をその節へ移した（2章の下から外した）

## 101.2 ★ 数値は計算し直して、渡された表と突き合わせた
- 9か国の本土の中心・面積：**一致**。ただし**ベルギーの経度は東経4.648度で、丸めると4.6度**（渡された表は4.7度）。順は変わらない
- 隣り合い：**一致**。ただし**フランスの域外の隣国は、Natural Earth の形のまま数えると5**（仏領ギアナでブラジル・スリナム）。
  記事は「本土で3、仏領ギアナでブラジル・スリナムとも接する」と両方書き、地図と訓練は本土の3を使う
- 海に面していない国：4つ（本土の海岸線がどれも0km）。リヒテンシュタインは二重内陸国（隣国2つがどちらも内陸）。
  ウズベキスタンの隣国5つもすべて内陸（カスピ海は海に数えない）
- cos φ：9か国の本土は北緯42.33〜54.91度で 1/cos φ は **1.353〜1.739**（渡された 1.35〜1.74 と一致）。モナコ1.384、
  オランダ 1.580〜1.680（中心 52.3度で1.634）。§06 の見比べる図：9か国の面積で**展開図は実際の約1.51倍**、右の正射図法は約0.6%小さい
- 地域のカードの残りのずれ：cos 49° / cos φ で、南の端 0.887倍・北の端 1.141倍
- ★ **モナコとリヒテンシュタインの面積は Natural Earth だと大きく外れる**：モナコ19km²（12の点の粗い形が海まで含む。実際は約2km²、
  2024年12月の埋め立て地マレテッラで少し増えた）、リヒテンシュタイン137km²（公式は160km²、リヒテンシュタイン統計局
  「Liechtenstein in Zahlen 2026」）。表には計算値に※を付け、脚注に公式の値を書いた。中心の経度のずれは0.1度未満で、
  隣の国との差（約0.8度）より小さいので順番には影響しない

## 101.3 ★ 首都の注記（オランダのほかに、スイス）
- **オランダ**：憲法上の首都はアムステルダム（憲法第32条。国王の即位を首都アムステルダムで行う、という1か所だけ）。政府と議会はハーグ
- ★ **スイス**：連邦憲法に首都の定めが無い。ベルンは「連邦都市」で、政府と議会の所在地であることは法律で定める
  （スイス連邦内閣府事務局の調査、swissinfo.ch）。オランダとは別の種類だが、同じ箱に並べた
- 参考：**ドイツ**は首都ベルリンに政府と議会があり、1994年のベルリン・ボン法で16の省のうち6つがボンを第一の所在地にしている。
  首都と政府の所在地が分かれてはいないので、箱の中で小さく「別の種類」として書いた
- 確認日はすべて2026年9月26日。地図の首都の点は Natural Earth の人口地点（ハーグは白抜きの丸で「政府・議会」、ベルンは「連邦都市」）
- ルクセンブルクとモナコは、国名と首都名が同じなので、首都の図のラベルを「ルクセンブルク市」「モナコ（都市国家）」にした
  （国名のラベルと同じ文字が2つ並ばないように）

## 101.4 国のページ9枚
- 範囲は国の本土（フランスはコルシカ島まで）に余白。フランスの5つの海外県とオランダのカリブ海の3つの特別自治体は図の外で、
  座標を注記に書いた（日本の南鳥島と同じ扱い。埋めない）
- ★ **ルクセンブルクの層3の「区」は2015年10月3日に廃止された区分**。Natural Earth v5.1.2 がこの区分のままなので、直さずに描き、
  注記に書いた（いまは国と100の基礎自治体のあいだに行政の段が無い）
- Geofabrik の抽出の日付は国ごとに出典欄に出る（9か国は2026-09-25 までのデータ。日本とチャドは09-24）

## 101.5 ★ 2-2・2-3 がまだ無いので、この回で使えないもの
2章は地球全体に1本で通る仕組み（cos φ・プレート・海流）、3章はその地域での現れ方、という分担（2-1 付録A）。いま書けているのは
cos φ（2-1）だけなので、**アルプス（プレート）と北大西洋海流はこの回に置けない**。2-2・2-3 を書いたら、**3章の書き終えた記事へ
遡って足す**。付録Aの箱に書いた

## 101.6 ★ Geofabrik の MD5 は「日付つきの名前」で突き合わせる
`*-latest.osm.pbf` は日付つきのファイル（`germany-260925.osm.pbf`）へ転送されるが、`*-latest.osm.pbf.md5` は**ミラー（GWDG）へ
転送され、ミラーが1日古いことがある**。ドイツで、正しく取れたファイル（MD5 `fd9bab7…` ＝ 日付つき 260925 の値と一致）を、
ミラーの 260924 の MD5 と比べて「合わない」と判定し、取り直しかけた。`extract.sh` を、転送先の日付つきの名前を先に確かめ、
そのファイルとその名前の `.md5` を Geofabrik から取って突き合わせるように直した（取った日付つきの名前は `dated.txt` に残す）。
キャッシュバスターは、3-1 の記事・国のページ・layout.js（CRUMBS）が入るので r93→r94（物理数学は r93 のまま。コミット前にもう一度確かめる）

## 101.7 ★ 川は `name` で数え、ラベルだけ `name:ja`（ライン川）／入りきらないラベルと「なし」を書き分ける／湖は国境で切らない
**川**：それまでは「`name:ja`、無ければ `name`」で**束ねて**いたので、一部の区間にだけ日本語名が入った川が割れていた
（ドイツのライン川が「Rhein」340km・6位と「ライン川」188km・21位、フランス国境の「Le Rhin / Rhein」の3つ）。
**同じ川かどうかは `name`（無ければ `name:ja`）で判定し、ラベルだけ `name:ja`** にした（塊に重なる区間の `name:ja` のうち長さが最大のもの）。
- **信濃川と千曲川は別のまま**（`name` 自体が違う）。付録の文も「名前そのものが変われば別」「同じ `name` なら日本語名が一部でも1本」を書き分けた
- **エルベ川が2回出るのはそのまま**（同じ名前でも1km 以上途切れている。つなぐ判断を入れると基準が揺れる）。付録に1文
- 作り直した11か国のうち、川の順位・長さが変わったのは5か国：
  - **ドイツ**：ドナウ川（608km）・ライン川（529km）が1・2位に入り、旧6位の「Rhein」と4位の「Donau」が消えた
  - **オーストリア**：ドナウ川（361km）が2位→1位（「Donau」302km が日本語名のある区間とまとまった）
  - **フランス**：ガロンヌ川 510→489km（3位→5位。`name` が「La Garonne」でない区間が抜けた）、「La Meuse」の塊のラベルが「マース川」（354km、14位）
  - **ベルギー**：マース川 132→140km（順位は2位のまま）。「Maas」（オランダ語の `name`）は別のまま
  - ★ **日本**：吉野川 200→196km（順位は10位のまま）。`name` が「吉野川 (Yoshinogawa)」の区間（4km）が別になった。
    `name` に2つの文字体系が並ぶ区間の扱いは、ラベルの文字体系の規則（§100.5）と違って**束ねる前には使わない**（束ねる判断を増やさない）
  - チャド・オランダ・ルクセンブルク・モナコ・スイス・リヒテンシュタインは変わらない。本数（15本など）はどの国も変わらない

**入りきらないラベル**：同じ縮尺を保つので、ベルギー（126px）・オランダ・スイスの図には名前が入りきらない（フランスは地域圏名13のうち6、
ベルギーは地域名3つとも出ない）。国のページの注記に「この縮尺で入りきらないラベルは置いていない。データには全部ある」と、
層3の名前がいくつのうちいくつ出ているかを書き、**ボタンの「なし」（データに無い）とは別**と明記した。数は図をまたいで名前で数える
（日本の鹿児島県が2枚の図に入るので、数で足すと47が48になった）。拡大しない理由は §100.11（同じ縮尺で並ぶことが土台）

**湖**：面積は国の形を約2km 広げた範囲で測るので、国境をまたぐ湖は隣の国の側の湖面も含む（レマン湖はフランスで347km²、
フランス側だけなら約234km²）。**国境で切ると、同じ湖が国によって違う大きさになる**ので切らない。付録に1文

---

# 102 2-2 プレートと大地形（2章 地球の2本目）

## 102.1 ★ データは5つ。出典とライセンスが分かれる
| 中身 | 出典 | ライセンス |
|---|---|---|
| プレートと境界・造山帯 | PB2002（Bird 2003, G3 4(3) 1027, doi:10.1029/2001GC000252）。GeoJSON 版は fraxen/tectonicplates | Open Data Commons Attribution License 1.0（ODC-BY） |
| 境界の7分類 | PB2002 の原データ `PB2002_steps.dat`（peterbird.name/oldFTP/PB2002/。GeoJSON 版は「沈み込み」しか区別しない） | 論文の補足資料（引用） |
| 山脈・高原・砂漠、陸の輪郭 | Natural Earth v5.1.2（ne_10m_geography_regions_polys・ne_50m_land。NAME_JA はすべて入っている） | パブリックドメイン |
| 地震 | USGS ANSS ComCat。M6.0以上・1900-01-01以降・種別 earthquake、14,455件（2026-09-26T06:11Z 取得。最後は 2026-09-25） | パブリックドメイン |
| 火山 | Smithsonian Global Volcanism Program, Volcanoes of the World（完新世の火山） | **非商業限定・引用必須** |

★★ **GVP の火山のデータだけ、非商業の利用に限られる。将来サイトに広告を入れる場合、この判断を見直す必要がある。
ほかの4つ（PB2002・Natural Earth・USGS）には制限が無い**（PB2002 の GeoJSON 版は ODC-BY で、表示すれば商業の利用も可）。
記事の出典欄と付録Bに書いた。

## 102.2 ★ PB2002_steps.dat の中身と、7分類→3つ
- 5,819行（ステップ）、各行15列。列は `PB2002_steps_desc.txt` のとおり：通し番号・続きの印・境界の組・始点と終点の経緯度・長さ・
  方位・相対速度・その方位・開く成分・右横ずれ成分・標高・海底の年代・続きの印・**分類**・**造山帯の印**
- ★ **分類の前の「:」と、境界の組の前の「:」は「前のステップの続き」という印で、分類ではない。分類の後ろの「*」は
  「ステップの中点が造山帯の中にある」という印**（1,022ステップ）。どちらも取り除いて7分類で数える
- 7分類の長さ（全260,483km）：OSR 67,335・SUB 51,309・OTF 47,785・CRB 27,470・CTF 26,133・CCB 23,003・OCB 17,448km
- 3つへのまとめ：**広がる＝OSR・CRB（94,805km、36%）、沈み込む＝SUB・OCB・CCB（91,760km、35%）、すれ違う＝OTF・CTF
  （73,918km、28%）**。「沈み込む」には、沈まずにぶつかる境界（CCB・OCB。ヒマラヤ）も入れた——3つにまとめると消える区別なので、
  記事ではヒマラヤに「衝突」と添えた
- 相対速度：境界の長さで重みをつけた中央値は年29.9mm、最大はトンガ海溝の北端（年263mm）。平均は SUB 62・OSR 47・CRB 18mm

## 102.3 数値（本教材が計算）
- プレート：52枚、合計5億1,010万km²（地球の表面積と一致）。**面積の上位7つが、指定の7つ（太平洋・南極・アフリカ・北アメリカ・
  ユーラシア・オーストラリア・南アメリカ）とちょうど一致し、合わせて81.0%**。8位ソマリア3.7%
- 造山帯13：合わせて地表の7.45%。ペルシア・チベット・ビルマ（1,659万km²）がいちばん大きい
- **地震の85.3%が境界の線から200km以内**（地表では17.1%＝約5倍の密度）。造山帯の中も含めると93.2%。M7以上は82.9%。
  200km以内の地震のうち、いちばん近い境界が「沈み込む」ものが65.3%。最大の4つ（1960チリ M9.5・1964アラスカ・2004スマトラ・
  2011東北）はすべて沈み込む境界
- 距離は、ステップを約5kmごとの点に刻んで KD 木で最近点を探した（球の上の弦の長さから大円距離へ）。地表の割合は球に一様な20万点
- ★ **山脈と境界は「形の輪郭の近さ」ではなく「面積のうち近い割合」で測った**：輪郭で測ると大きな形ほど有利になり、砂漠が59%と
  境界に近いように出てしまった。0.25度の格子で面積を重みづけすると、「線から200km以内または造山帯の中」は陸地全体24.7%・
  **山脈48.7%**・高原18.1%・**砂漠16.0%**（陸地より少ない＝砂漠は境界と関係が無い、の裏づけ）
- ★ **アルプス山脈は境界の線から約860km**（山脈の97%が「Alps」造山帯の中）。PB2002 はアフリカとユーラシアの境界の線を地中海に引き、
  アルプスの衝突は広い帯として造山帯にしている。「分けられない」の説明にそのまま使った。アンデスは海溝（SUB）から内陸へ中央値約340km、
  ヒマラヤは境界から中央値約130km。ウラル（約1,860km）・アパラチア（約1,620km）はいまの境界からは説明できない山脈として書いた
- 05 の代表的な場所は、いちばん近いステップの分類が表の分類と一致することを確かめて置いた。最初の2つは合わなかったので動かした
  （大西洋中央海嶺の赤道付近は横ずれ断層 OTF がいちばん近い→北緯30度・西経42度へ。東アフリカ大地溝帯のケニアは CRB の線から
  622km→PB2002 の CRB がある北緯5.3度・東経36.5度へ）

## 102.4 記事（`html/memoryverse_2-2.html`）とカード
- 型は 2-1。`<style>` は 2-1 から使う部品だけ（3分類の `.known` を含む）。CRUMBS に `memoryverse_2-2` を 2-1 の次に足した
  （前後リンクは 2-1 ↔ 2-2 ↔ 3-1）
- カード3枚は `tools/memoryverse-cards/earth.py` が描く。**枠は 2-1 と同じ全球の正距円筒図法**（左端が西経180度、横2・縦1。
  2-1 の 2048×1024 を 1024×512 で）で、30度の升目とそのラベル（30°E・30°N）も 2-1 と同じ書き方。部品は `country-card.css/js`
  - 04 プレート：層1 大きい7つ／層2 残りの小さいプレート／層3 境界の線（3つの動きで色分け）と造山帯（斜線。埋めない）
  - 06 地震と火山：層1 地震（M7以上は大きい点）／層2 火山／層3 境界の線。14,455件の点を1本のパス（丸い線端の長さ0の線）に
    まとめたので、下限を上げずに全件を描けた（SVG 約300KB）
  - 07 大地形：層1 山脈／層2 高原／層3 砂漠。境界の線は薄く下敷き（層に入れない）
  - 05 の図（3つの動きと代表的な場所）は記事に埋め込む（約59KB。境界のステップを線につないで小さくした）
- 日本の下は PB2002 ではオホーツク（東京・札幌）・アムール（大阪）・沖縄（那覇）・フィリピン海（八丈島）。教科書の
  北アメリカ・ユーラシアとの違いを箱に書いた

## 102.5 ★ 3分類
分かっていること＝境界の位置・地震と火山の分布・動く速さ（GPS）。まだ確かめられていない＝プレートを動かす仕組みの詳細。
まだ誰も見ていない＝マントルの中、海底の大部分——**2-1 §04 の「現代の基準で測られた海底は約28.7%」につないだ**
（広がる境界＝海嶺の大部分が、測られていない海底を通っている）

## 102.6 ★ 遡って足す必要があるもの
- **3-1 西ヨーロッパにプレートの軸を足す**（アルプス＝アフリカとユーラシアの衝突、アルプス造山帯）。3-1 の付録A・§98 に対応
- 2-3（気候と海流）はまだ案。砂漠の位置（2-2 §07 で「この軸では説明できない」とした）と、3-1 の北大西洋海流は 2-3 を書いてから

- キャッシュバスター：2-2 の記事・カード・layout.js（CRUMBS）が入るので r95→r96（物理数学が r95 まで使っている。コミット前にもう一度確かめる）

## 102.7 ★ 火山は「取得待ち」で公開した（「なし」とは別の状態）
- 2026-09-26 15:10 から Smithsonian GVP のサイトが「temporarily unavailable」（403）で、WFS も接続を切られた。5分ごとに取り直したが
  18:00 まで戻らなかったので、**火山を空けたまま**公開した。**代わりの出典（第三者の再配布）は使わない**——GVP と同等の一覧は無く、
  コピーを使うと版と出典が濁り、付録Bの「同じデータと手順で誰でも再現できる」が守れなくなる
- ★ **「取得待ち」は「なし」と区別する**。「なし」（§100.4）は**データに無い**、「取得待ち」は**データはあるが、まだ取れていない**。
  見た目も分けた：`css/country-card.css` の `.cc-pending`（点線の枠・薄く・琥珀色の「取得待ち」）と、`.cc-zero`（破線の枠・赤い「なし」）。
  `js/country-card.js` は、押せない層のボタン（`cc-lay` の disabled）を切り替えないようにした（層は消えたまま）
- 記事：§06 に「この回では地震だけで示す。火山の分布も境界に沿って並ぶが、データを取れていないため後で足す」の箱。§01 到達目標③を
  「地震と山脈」に。§06 のカードの注記・表の火山の行・出典欄・付録Bに「取得待ち」
- ★ **付録Aに、未完であることと、足すときに直す箇所の一覧**を書いた（§06 のカードの層2と注記、§06 の表、§06 の箱を外す、
  §01 到達目標③を戻す、GVP の引用を版と取得日つきで書く、付録Aの箱を消す）
- GVP が戻ったら、火山を足す**別のコミット**にする。手順：GVP の WFS から完新世の火山の CSV を取り（版は GVP の引用の書式に従う）、
  `data/earth/gvp_holocene.csv` に置いて `earth.py 2-2` を走らせ、記事の上の一覧を直す

---

# 103 複合分野と「地球と人類」の器（記事はまだ無い）

## 103.1 ★ 階層と、置いたもの
勉強の軌跡 > **複合分野**（`study_composite.html`）> **地球と人類**（`earthhuman.html`、見出しは「地球の構造から人類を読み解く」）> 第I部 地理 > 第1章。
- CRUMBS に `composite`（parent: study）と `earthhuman`（parent: composite、unit: 章）の2行
- ★ `earthhuman` に `title: '地球の構造から人類を読み解く'` を付ける指示だったが、**付けていない**。ツリー（`treeHtml`）は
  title を label より優先するので、付けるとツリーが「複合分野 > 地球の構造から人類を読み解く」になり、「地球と人類」と出ない。
  CRUMBS の title がほかに効くのは前後リンクの名前だけで、シリーズの行には前後リンクが無い。長い名前は一覧ページの h1 が持つ
- `study.html` の学問分野と精神の圏域のあいだに「複合分野」の節（h2）を足し、カードは「地球と人類」1枚。新しいスタイルは足していない
- ★ **このカードは `earthhuman.html` へ直接降りる**（複合分野の一覧を1段飛ばす）。§49.3「カードは1段だけ下へ降りる」の**例外**で、
  指示どおりにした。複合分野の一覧へは、サイト構成ナビとシリーズ一覧の「← 一つ上」から行ける。複合分野のシリーズが2つ以上になったら、
  §49.3 に戻して `study_composite.html` へ降りるカードに変えるかを決める
- `study_composite.html` は `study_skills.html` と同じ作り（カード1枚、h2 なし）
- `earthhuman.html` は `phys-math.html` と同じ「部の見出し（h2）＋章のカード」。7部35章。記事が無いので章のカードは**すべて div**
  （`markSoonCards` が「準備中」を付ける）。記事ができたら a にして href を足し、CRUMBS に章を登録する
- 一覧ページだが、章の前に教材の設計（6層の図・地理決定論の箱・手筋 U1〜U8・ペース・1章の流れ・併読書・この教材について）を
  文章で置くので、そのぶんの `<style>` をページに持つ（一覧ページは `lesson-theme.css` を読まない）。箱の琥珀は 2-1 の `.box.warn` と同じ値
- ★ 第IV部（地政学）には「事実の争いと価値の争いを分け、各当事者の主張を内在的論理として並べ、結論は書かない」の箱を部の頭に置いた

## 103.2 ★★ メモリーバースとの分担：「なぜ」と「どこ」
| | 受け持つ問い | 中身 |
|---|---|---|
| **地球と人類** | **なぜ**そこにそうあるのか | 物理の式、仮想の大陸、推論の手筋（U1〜U8） |
| **メモリーバース** | **どこ**に何があるか | 升目（30度）、実際のデータの分布、白地図に描く訓練 |

- ★ **同じ事実を2か所に書かない。**片方にだけ書き、もう片方からはそこへリンクして参照する。どちらに書くかは上の表の問いで決める
  （仕組みの説明は地球と人類、場所と数値の一覧はメモリーバース）
- `earthhuman.html` の「なぜこの6分野・この順番か」の末尾に、この分担の1文とメモリーバースへのリンクを置いた

## 103.3 ★ いま重なっているところ（地球と人類の第1章を書くときに片付ける）
| 地球と人類 | メモリーバース | 状態と、やること |
|---|---|---|
| 第1章 §05 プレート境界の3つの動き | **2-2** プレートと大地形（公開済み、§102） | 2-2 が3つの動き（離れる・近づく・すれ違う）を仕組みから説明している。★ **第1章を公開したら、2-2 の説明を短くして第1章への参照に置き換える**（2-2 に残すのは境界の**場所**：図・表・日本の下のプレート） |
| 第1章 §06〜§08 気候・海流・山脈 | **2-3** 気候と海流（**書いた**、§104） | ✓ 分担に沿って書いた。2-3 は「どこ」だけで、「なぜ」の5か所（§02・§04・§05・§07・§09）から「地球と人類 第I部 第1章（準備中）」へリンク（行き先はいま `earthhuman.html`）。第1章を公開したら、リンクの行き先を第1章の記事に直す |

- 第1章と 2-3 のどちらを先に書くかで参照の向きが決まる。先に書いた側に説明を置き、後から書く側は参照にする。
  2-2 は先に書いてしまったので、上の表のとおり後から縮める
- §98 の連動の一覧にも1行足した

## 103.4 State
キャッシュバスター r97→r98（layout.js の CRUMBS の変更ぶん。コミットの直前に物理数学が r97 までなのを確かめた）。

---

# 104 2-3 気候と海流（2章 地球の3本目、2章の最後）

## 104.1 ★ 扱う範囲と、データ
- **「どこにあるか」だけ**（§103）。なぜは「地球と人類」第I部 第1章（準備中）へ送る。記事の5か所（§02・§04・§05・§07・§09）から
  `earthhuman.html` にリンクし、「地球と人類 第I部 第1章（準備中）」と書く（行き先が無いまま「ここでは扱わない」と書かない）
- データは4つ。**気温・降水量は CRU TS 4.09 の1本**（PREC/L〔CC0〕も使えたが、出典を1つに絞る。2-2 で出典が5つに分かれて付録が長くなったため）
  | データ | 利用条件 | 用途 |
  |---|---|---|
  | CRU TS 4.09（1991〜2020年の平年値、0.5度） | OGL v3（商業・再配布可、表示が条件。表示は「Climatic Research Unit (University of East Anglia) and Met Office」） | 気温・降水量・ケッペン |
  | NOAA Global Drifter Program 気候値 **版3.10**（ERDDAP drifter_annualmeans） | CC BY 4.0（メタデータの license） | 海流 |
  | ETOPO 2022 60秒（5つおき） | CC0 1.0 | 標高1000m以上を分けるだけ |
  | Natural Earth v5.1.2 | パブリックドメイン | 陸の輪郭・地名 |
- ★ **使わなかったもの**：WorldClim（商業・再配布が禁止）、Beck ほか2018のケッペン図（WorldClim から作られているので、再現する人が制限に当たる。自前の分類との一致は91.7%だったが教材には載せない）、
  GHCN_CAMS（熱帯に年平均51℃などあり得ない値。CRU と突き合わせて見つけた）、Esri の海流の線（出典が無い手描きの模式図）、ERA5（使えるが CDS のアカウントが要る）
- ★ **漂流ブイは版3.10のまま**：版3.11（2024年12月31日まで）が出ているが、引用した URL から同じものを取り直せることを優先した。観測の終わりは2023年2月28日（データの記載）。
  **始まりは確認できていない**（論文の版は1979年2月〜2015年6月。版3.10 には記載が無い）。論文の方法からフィルタが変わり、高緯度で流れが強く出るとされる——付録Bにそのまま書いた
- ★ 途中で WebFetch の要約が「ETOPO は public domain」と書いたが、元のページにその文言は無かった。利用条件は必ず原文（ここではメタデータの CC0-1.0）で確かめる

## 104.2 ★ 帯
- 緯度0.5度ごとの陸の**中央値**（気温と、対数をとった降水量）を標準化し、半球ごとに、帯の中のばらつき（陸の面積で重み）が最小になる切り方を動的計画法で出した
- **北 15.5・32・45.5・64度**（5本、90.4%）、**南 18.5・34度**（3本、82.5%）。`climate.py` は計算した境目が記事の値と一致することを assert で確かめる
- **名前は本教材が付けたもの**：北 熱帯／乾いた帯／中緯度の帯／冷たい帯／極の帯、南 熱帯／乾いた帯／温和な帯。
  ★ 北緯32〜45.5度は B 39%・D 30%・C 28% で**どれも過半に届かない**ので、性格ではなく位置で「中緯度の帯」。名前で性格を言えない帯があることを隠さない
- ★ **回帰線（23.44度）は帯の境目ではない**。どちらの回帰線も乾いた帯の中。2-1 §05（`memoryverse_2-1.html#lines`）へリンク

## 104.3 ★ 平均と中央値——比べる相手はどちらも中央値
- 北緯20〜30度：平均520〜625mm、中央値114〜174mm、半分以上（54〜57%）の土地が250mm未満。モンスーンアジア（東経70〜125度）はこの緯度の陸の33%で、雨の75%。除くと平均は569→209mm
- **降水量も気温も、同じ緯度（0.5度）の陸の中央値と比べる**。平均で比べた結果は**対照**として並べる。
  ★ 気温を平均のままにしなかった理由：主の版（標高1000m未満）は、中央値にしても**場所が変わらず値の差は0.3℃以下**（12→11 は、中国南部の2つのかたまりがつながっただけ）。揃えないほうが不自然
- ★ **1つの統計量の性質が2か所に出る**ことを本文で明示した：降水量はモンスーンアジアが平均を押し上げ、気温は高地が平均を引き下げる。どちらも「少数の極端な値に平均が引っぱられる」

## 104.4 ★ 外れ値
- 降水量：中央値の1/3以下。**中央値で13か所（主）**、平均で10か所（対照。サハラ〜アラビア〜アフリカの角〜イラン〜トゥランが1つの塊 1,744万km²）
- 気温：中央値との差±5℃以上、**標高1000m以上の陸を除く**（陸の81.1%が残る）。**11か所（主）**、全部の陸で26か所（対照。寒い側18は高地が先頭：チベット −14.0℃・アンデス −12.2℃）
- ★ 全部の陸を**平均**と比べると、暖かい側が14か所になり、イベリア・イタリア・サハラ北部・サハラ西部・イラン・エーゲ海のまわりが出る。**中央値ではこの6か所が消え、メソポタミア・トゥラン・パンジャーブ〜タール・ソノラは残る**
  （★ 報告の途中で「中央値ではメソポタミアだけが残る」と書いたのは誤り。正しい一覧で書いた）
- まとまりは10万km²以上。呼び名は本教材が付けたもの（`climate.py` の NAMES。Natural Earth の地名から）
- ★ **北西ヨーロッパは +7.3℃**（中央値・1000m未満）。3-1 の国では、ベネルクス3国は全部、ドイツ77%、フランス43%が入り、スイス・オーストリアは入らない（大部分が1000m以上）

## 104.5 ★ ケッペンと帯（§08）——外れ値と食い違いは別物
- 分類は CRU TS から自前で計算：Peel, Finlayson & McMahon (2007) HESS 11:1633 の Table 1 の主分類。**B を最初に決める**（同 p.1637「the B climates must be identified first」）。
  C/D の境は最寒月0℃。夏＝暖かい方の半年（AMJJAS か ONDJFM）。割合は A 23.4・B 30.4・C 14.0・D 25.8・E 6.4%
- 一致 **66.3%**（B を除くと61.3%）、緯度だけの上限 **70.0%**
- ★★ **最初の結論「食い違いは外れ値の場所と重なる」はデータと違った**（書く前に確かめて止めた）。外れ値の場所（陸の16.4%）の中の食い違いは29.4%で、それ以外（34.6%）より少ない。
  食い違いの 65.4% は、**帯の境目から3度以内**（食い違う割合46.3%、それ以外27.8%）か、**過半の分類が無い2つの帯**（中緯度の帯・極の帯）にある。
  記事は「外れ値＝その緯度にしては量が異常」「食い違い＝帯が予想する分類と違う」で**測っているものが違う**と書いた（例：サハラの中心部は極端な外れ値だが、乾いた帯で B と予想され実際 B）
- 副題は「緯度で7割、残りの3割はどこか」。「残りは海流と地形」はなぜの話で §103 に反し、事実とも違った

## 104.6 ★ 海流
- 1平方度あたり5日未満の観測の区域は使わない → 0.5度に平均 → 3×3 でならす → 等間隔の流線（間隔2.4度、0.05 m/s 以上、3度以上の長さ）。線の太さと明るさが速さ
- **名前**：教科書で使われる25の名前を候補にし、候補の場所から4度以内で、言われる向き（±45度）に流れる最も速いところが **0.10 m/s 以上**なら付ける（21）。
  0.15 だと北大西洋海流・ベンゲラ・親潮・北赤道海流（大西洋）が落ちるが、幅の広い遅い流れなので 0.10。図の注記にその理由を書いた。
  ★ **ペルー海流は 0.071 で付けない**（指示は「入れる」だったが規則と食い違ったので止めて報告し、規則どおりにした。チリ沿岸の狭い帯だけ 0.15 に届くが、それを使うとこの1本だけ規則が変わる）。
  同じ名前（赤道の3本）は海の名前を添えた
- **線の無い海域**：斜線＝観測が無い（海の11.6%。北極海・南極大陸のまわり・インドネシアの多島海・ハドソン湾・オホーツク海など）。斜線の無い空白＝観測はあるが 0.05 m/s より遅い（27.8%）。カスピ海は湖なので海から外した
- 赤道の南北で逆向きの流れが並ぶところ（ヘアピン）は、事実なので残した
- SVG は 670KB → **174KB**：ほとんどが点の間引き（0.5px）と相対座標。線の間隔・長さは変えていない

## 104.7 カードと図
| カード | 層1 | 層2 | 層3 | 大きさ |
|---|---|---|---|---|
| §04 `e2-3-bands.svg` | 年降水量（1度で塗る） | 等温線 0・10・20℃ | 帯の境目と回帰線 | 157KB |
| §06 `e2-3-dry.svg` | 中央値で比べる（主） | 平均で比べる（対照） | 帯の境目 | 96KB |
| §07 `e2-3-temp.svg` | 1000m未満で比べる（主） | 全部の陸（対照） | 1000m以上の高地 | 141KB |
| §08 `e2-3-koppen.svg` | ケッペン5分類（1度） | 帯が予想する分類 | 食い違う場所 | 161KB |
| §09 `e2-3-currents.svg` | 流線 | 名前（21） | 観測の無い海域 | 174KB |
- §05 の図（緯度ごとの平均・中央値・250mm未満の割合）は `data/e2-3-profile.svg` を記事に埋め込む（色は記事の CSS 変数）
- すべて `tools/memoryverse-cards/climate.py` が作る。記事の数値は `data/e2-3-numbers.json`

## 104.8 同じコミットで直したもの
- **2-2 の4か所**（2-3 で「なぜ」を説明すると約束していた箇所。§103 と食い違っていた）：§07 の砂漠の箱（「教科書では20〜30度とされるが、データから引くと北緯15.5〜32度・南緯18.5〜34度（2-3）」）、
  演習4の解答(5)、付録Aの「2-3 はまだ案」、付録Cの予告。★ 指示は3か所だったが、演習4の解答(5)にも同じ約束があったので直して報告した
- ★ **一覧ページ（memoryverse.html）に 2-2 のカードが無かった**——2-2 のコミットの確認項目に「一覧ページのカード」が無かったため。2-3 のカードと一緒に足した。
  **今後は、記事を足すときの確認項目に「一覧ページのカード」を入れる**

## 104.9 State
キャッシュバスター r99→r100（2-3 の記事・カード・layout.js の CRUMBS。コミットの直前に物理数学が r99 までなのを確かめた）。
